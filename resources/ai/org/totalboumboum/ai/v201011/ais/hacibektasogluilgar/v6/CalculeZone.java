...
public class CalculeZone {
	...
	public CalculeZone(HacibektasogluIlgar hi) throws StopRequestException {
		...
		bonuses = new AiItem[zone.getItems().size()
				+ zone.getHiddenItemsCount()];
	 ...
	 * 		description manquante !
	 */
	public void initialiserMatrices(Etats matrice[][])
			throws StopRequestException {
		hi.checkInterruption();
	...
	public void calculeMurs(Etats matrice[][]) throws StopRequestException {
		...
		while (iterMur.hasNext()) {
			...
			if (mur.isDestructible()) {
				...
				matrice[mur.getLine()][mur.getCol()] = Etats.MUR_NON_DESTRUCTIBLE;
	 ...
	 * 		description manquante !
	 */
	private Direction direction(AiTile source, AiTile cible, int longueur)
			throws StopRequestException {
		hi.checkInterruption();
		Direction resultat;
		...
		if (source.getLine() - cible.getLine() == 0) {
			...
			if (longueur > 0)
				resultat = Direction.LEFT;
			else
				resultat = Direction.RIGHT;
		} else {
			longueur = source.getLine() - cible.getLine();
			if (longueur > 0)
				resultat = Direction.UP;
			else
		...
		while (iterBombes.hasNext()) {
			...
			for (int i = 0; i < inScopeTiles.size(); i++) {
				...
				if (bombe.getTime() < bombe.getNormalDuration() / 3
						&& matrice[inScopeTiles.get(i).getLine()][inScopeTiles
								.get(i).getCol()] != Etats.FLAMMABLE_ROUGE
						&& matrice[inScopeTiles.get(i).getLine()][inScopeTiles
								.get(i).getCol()] != Etats.FLAMMABLE_NOIR) {
					if (matrice[inScopeTiles.get(i).getLine()][inScopeTiles
							.get(i).getCol()] == Etats.BONUS) {
						matrice[inScopeTiles.get(i).getLine()][inScopeTiles
								.get(i).getCol()] = Etats.FLAMMABLE_VERT_BONUS;
						int longueur = 0;
						Direction direction = direction(bombe.getTile(),
								inScopeTiles.get(i), longueur);

						for (int a = 0; a < bombe.getRange()
								- Math.abs(longueur); a++) {
							hi.checkInterruption();
							AiTile voisine = inScopeTiles.get(i).getNeighbor(
									direction);
						...
						matrice[inScopeTiles.get(i).getLine()][inScopeTiles
								.get(i).getCol()] = Etats.FLAMMABLE_VERT;

				} else if (bombe.getTime() < bombe.getNormalDuration() * 2 / 3
						&& matrice[inScopeTiles.get(i).getLine()][inScopeTiles
								.get(i).getCol()] != Etats.FLAMMABLE_NOIR) {
					if (matrice[inScopeTiles.get(i).getLine()][inScopeTiles
							.get(i).getCol()] == Etats.BONUS) {
						matrice[inScopeTiles.get(i).getLine()][inScopeTiles
								.get(i).getCol()] = Etats.FLAMMABLE_ROUGE_BONUS;
					...
					if (matrice[inScopeTiles.get(i).getLine()][inScopeTiles
							.get(i).getCol()] == Etats.BONUS) {
						matrice[inScopeTiles.get(i).getLine()][inScopeTiles
								.get(i).getCol()] = Etats.FLAMMABLE_NOIR_BONUS;
	 ...
	 * 		description manquante !
	 */
	public void calculeAdversaire(Etats matrice[][])
			throws StopRequestException {
		hi.checkInterruption();
	...
	/**
	 * on fait mis à jour de tous les matrices calculé pour avoir total matrice
	 * du zone
	 * 
	 * @param matrice
	 * 		description manquante !
	 ...
	 * 		description manquante !
	 */
	public void misAJourEtatsMatrice(Etats matrice[][])
			throws StopRequestException {
		hi.checkInterruption();
	...
	/**
	 * methode affichante la matriceTotale en console et en zone de jeu à la
	 * fois
	 * 
	 * @param matriceTotal
	 * 		description manquante !
	 ...
	 * 		description manquante !
	 */
	public void afficherMatrice(double matriceTotal[][], Etats matriceEtat[][])
			throws StopRequestException {
		hi.checkInterruption();
