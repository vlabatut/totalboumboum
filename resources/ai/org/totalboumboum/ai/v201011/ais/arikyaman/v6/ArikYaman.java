...
public class ArikYaman extends ArtificialIntelligence {
	...
	public AiAction processAction() throws StopRequestException {
		...
		if (onetime) {
			...
			this.refreshMatrix(ProcessMatrix);
		}
		int n = 0;
		if (GameMap.getHiddenItemsCount() != 0
				&& (AY.getBombNumberMax() + AY.getBombRange())
						/ GameMap.getHiddenItemsCount() < 1 / 2) {
			n = 100;
		} else {
			n = 1;
		}
		...
		try {
			AiTile opt = this
					.FindClosestOptimum(ProcessMatrix, AY.getTile(), 0);
			...
			if (ProcessMatrix[AY.getLine()][AY.getCol()] < 0) {
				if (escape && !Safes.isEmpty()) {
					if (path2 != null) {
						...
						if (count > 2
								|| ProcessMatrix[path2.getLastTile().getLine()][path2
										.getLastTile().getCol()] <= 0
								|| this.checkPathCorruption(path2,
										ProcessMatrix)) {
				...
				if (path2 == null
						|| this.checkPathCorruption(path2, ProcessMatrix)) {
					if (!AccesiblePositives.isEmpty()) {
						if ((opt == AY.getTile() && canbomb)) {
							path2 = this.calculeShortestPath(
									AccesiblePositives, GameMap);
							canbomb = false;
							pathrecord = 1;
						} else {
					...
					try {
						if ((ProcessMatrix[path2.getLastTile().getLine()][path2
								.getLastTile().getCol()] <= 0 && escape != false)
								|| closed) {
							path2 = this.calculeShortestPath(opt, GameMap);
							pathrecord = 1;
						}
			...
			if (path2 != null) {
				if (path2.getLastTile() == AY.getTile()) {
					...
					if (s > 0 && s < 72
							&& this.CanDropBomb(ProcessMatrix, AY, GameMap)) {
					...
					Direction direction = this.DirectionChooser(path2, AY,
							pathrecord);
	...
	private Direction DirectionChooser(AiPath path, AiHero hero, int pathrecord)
		...
		if (ProcessMatrix[path.getTile(pathrecord).getLine()][path.getTile(
				pathrecord).getCol()] < -165
				&& ProcessMatrix[hero.getLine()][hero.getCol()] >= 0) {
	 ...
	 *             description manquante !
	 */
	private boolean CanDropBomb(int[][] ProcessMatrix, AiHero AY, AiZone GameMap)
		...
		for (int i = 1; i <= range; i++) {
			checkInterruption();
			if (DuplicateMatrix[(line + i + maxline) % maxline][col] > -100
					&& dp) {
				DuplicateMatrix[(line + i + maxline) % maxline][col] = -100;
			} else {
				if (DuplicateMatrix[(line + i + maxline) % maxline][col] <= -900) {
					dp = false;
				}
			}
			if (DuplicateMatrix[(line - i + maxline) % maxline][col] > -100
			...
			if (DuplicateMatrix[line][(col + i + maxcol) % maxcol] > -100 && lp) {
				...
				if (DuplicateMatrix[line][(col + i + maxcol) % maxcol] <= -900) {
					lp = false;
				}
			}
		...
		if (Safes.isEmpty()) {
			...
			if ((duration - 500) / 1000 < (isescape.getPixelDistance() / speed)) {
				...
				List<AiTile> list = isescape.getTiles();
	...
	private void fillwithAtackPlaces(int[][] ProcessMatrix, AiZone GameMap,
			int coef) throws StopRequestException {
		checkInterruption();
		...
		while (itallheroes.hasNext()) {
			...
			if (hero != AY) {
				...
				for (int i = 1; i <= AY.getBombRange(); i++) {
					checkInterruption();
					if (r
							&& ProcessMatrix[hero.getLine()][hero.getCol() + i] >= 0) {
						ProcessMatrix[hero.getLine()][hero.getCol() + i] = 32
								- this.distanceCoefCalc(hero.getLine(),
										hero.getCol() + i, hero.getTile(),
										ProcessMatrix, 32)
								+ coef
								* this.distanceCoefCalc(hero.getLine(),
										hero.getCol() + i, AY.getTile(),
										ProcessMatrix, 16);
					} else {
						if (r
								&& ProcessMatrix[hero.getLine()][hero.getCol()
										+ i] <= -900) {
							r = false;
						}
					}
					...
					if (d
							&& ProcessMatrix[hero.getLine() + i][hero.getCol()] >= 0) {
						ProcessMatrix[hero.getLine() + i][hero.getCol()] = 32
								- this.distanceCoefCalc(hero.getLine() + i,
										hero.getCol(), hero.getTile(),
										ProcessMatrix, 32)
								+ coef
								* this.distanceCoefCalc(hero.getLine() + i,
										hero.getCol(), AY.getTile(),
										ProcessMatrix, 16);
					} else {
						if (d
								&& ProcessMatrix[hero.getLine() + i][hero
										.getCol()] <= -900) {
							d = false;
						}
					}
		...
		for (int i = 0; i < ProcessMatrix.length; i++) {
			...
			for (int j = 0; j < ProcessMatrix[i].length; j++) {
				...
				if (ProcessMatrix[i][j] == -900) {
					int ip1 = ((i + 1) % ProcessMatrix.length), im1 = (i - 1 + ProcessMatrix.length)
							% ProcessMatrix.length;
					int jp1 = ((j + 1) % ProcessMatrix[i].length), jm1 = (j - 1 + ProcessMatrix[i].length)
							% ProcessMatrix[0].length;
					if (ProcessMatrix[ip1][j] == 0) {
						ProcessMatrix[ip1][j] += 20 - this.distanceCoefCalc(
								ip1, j, tile, ProcessMatrix, 20);
					}
					if (ProcessMatrix[im1][j] == 0) {
						ProcessMatrix[im1][j] += 20 - this.distanceCoefCalc(
								im1, j, tile, ProcessMatrix, 20);
					}
					if (ProcessMatrix[i][jp1] == 0) {
						ProcessMatrix[i][jp1] += 20 - this.distanceCoefCalc(i,
								jp1, tile, ProcessMatrix, 20);
					}
					if (ProcessMatrix[i][jm1] == 0) {
						ProcessMatrix[i][jm1] += 20 - this.distanceCoefCalc(i,
								jm1, tile, ProcessMatrix, 20);
					}
				}
			}
		}
	...
	private void fillwithBombs(int[][] ProcessMatrix, AiZone GameMap)
		...
		for (int i = 0; i < bmbs.length; i++) {
			...
			double timecoef;
			timecoef = (double) (bmb.getNormalDuration() - bmb.getTime())
					/ bmb.getNormalDuration();
			if (ProcessMatrix[bmb.getLine()][bmb.getCol()] > -200 + 100
					* timecoef) {
			...
			while (itblasts.hasNext()) {
				...
				if ((blstzn.getLine() != bmb.getLine() || blstzn.getCol() != bmb
						.getCol())
						&& ProcessMatrix[blstzn.getLine()][blstzn.getCol()] > -900) {
					if (ProcessMatrix[blstzn.getLine()][blstzn.getCol()] > ProcessMatrix[bmb
							.getLine()][bmb.getCol()] + 1) {
						ProcessMatrix[blstzn.getLine()][blstzn.getCol()] = ProcessMatrix[bmb
								.getLine()][bmb.getCol()] + 1;
					}
				}
			}
	...
	private void fillwithBns(int[][] ProcessMatrix, AiZone GameMap, int n)
		...
		while (itbns.hasNext()) {
			...
			ProcessMatrix[bns.getLine()][bns.getCol()] += n
					- this.distanceCoefCalc(bns.getTile(), this.AY.getTile(),
							ProcessMatrix, n);
		}
	}
	 ...
	 *             description manquante !
	 */
	private int distanceCoefCalc(AiTile tile1, AiTile tile2, int[][] matrix,
			int n) throws StopRequestException {
		checkInterruption();
		double a = Math.sqrt(matrix.length * matrix.length + matrix[0].length
				* matrix[0].length);
		double b = Math.sqrt((tile1.getLine() - tile2.getLine())
				* (tile1.getLine() - tile2.getLine())
				+ (tile1.getCol() - tile2.getCol())
				* (tile1.getCol() - tile2.getCol()));
	 ...
	 *             description manquante !
	 */
	private int distanceCoefCalc(int x, int y, AiTile tile2, int[][] matrix,
			int n) throws StopRequestException {
		checkInterruption();
		double a = Math.sqrt(matrix.length * matrix.length + matrix[0].length
				* matrix[0].length);
		double b = Math.sqrt((x - tile2.getLine()) * (x - tile2.getLine())
				+ (y - tile2.getCol()) * (y - tile2.getCol()));
		int result = (int) ((b / a) * n);
		return result;
	}
	...
	private Object[] sortBombs(Object[] bmbs) throws StopRequestException {
		...
		for (int i = 0; i < bmbs.length; i++) {
			...
			for (int j = i; j < bmbs.length; j++) {
				...
				if ((((AiBomb) bmbs[j]).getNormalDuration() - ((AiBomb) bmbs[j])
						.getTime()) < lower) {
					lower = ((AiBomb) bmbs[j]).getNormalDuration()
							- ((AiBomb) bmbs[j]).getTime();
					lowest = j;
				}
			}
			temp = bmbs[i];
	 ...
	 *             description manquante !
	 */
	private AiTile FindClosestOptimum(int[][] ProcessMatrix, AiTile Tile,
			int param) throws StopRequestException {
		checkInterruption();
		...
		if (ProcessMatrix[linep1][col] > -200 && param != 2) {
			if (tiles.contains(GameMap.getTile(linep1, col))) {
				...
				if (ProcessMatrix[linep1][col] > 0) {
					if (!AccesiblePositives.contains(GameMap.getTile(linep1,
							col))) {
				...
				tile[0] = this.FindClosestOptimum(ProcessMatrix,
						GameMap.getTile(linep1, col), 1);
			}
		...
		if (ProcessMatrix[linem1][col] > -200 && param != 1) {
			if (tiles.contains(GameMap.getTile(linem1, col))) {
				...
				if (ProcessMatrix[linem1][col] > 0) {
					if (!AccesiblePositives.contains(GameMap.getTile(linem1,
							col))) {
				...
				tile[1] = this.FindClosestOptimum(ProcessMatrix,
						GameMap.getTile(linem1, col), 2);
			}
		...
		if (ProcessMatrix[line][colp1] > -200 && param != 4) {
			if (tiles.contains(GameMap.getTile(line, colp1))) {
				...
				if (ProcessMatrix[line][colp1] > 0) {
					if (!AccesiblePositives.contains(GameMap.getTile(line,
							colp1))) {
				...
				tile[2] = this.FindClosestOptimum(ProcessMatrix,
						GameMap.getTile(line, colp1), 3);
			}
		...
		if (ProcessMatrix[line][colm1] > -200 && param != 3) {
			if (tiles.contains(GameMap.getTile(line, colm1))) {
				...
				tile[3] = this.FindClosestOptimum(ProcessMatrix,
						GameMap.getTile(line, colm1), 4);
			}
		}
	 ...
	 *             description manquante !
	 */
	private AiPath calculeShortestPath(AiTile Tile, AiZone GameMap)
			throws StopRequestException, LimitReachedException {
		checkInterruption();
	 ...
	 *             description manquante !
	 */
	private AiPath calculeShortestPath(List<AiTile> Tiles, AiZone GameMap)
			throws StopRequestException, LimitReachedException {
		checkInterruption();
	 ...
	 *             description manquante !
	 */
	private boolean checkPathCorruption(AiPath path, int[][] ProcessMatrix)
			throws StopRequestException {
		checkInterruption();
