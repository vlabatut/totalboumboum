	...
	protected boolean considerBombing() throws StopRequestException {
		...
		myhero = zone.getOwnHero();
		{
			...
			if (it1.hasNext()) {
				...
				// Si le tile que l'agent arrive ne possede des bonus , ça veut
				// dire que l'agent vient pour la détruire.
				if (tiles.get(0).getItems().isEmpty()) {
					ai.checkInterruption();

					if (tiles.get(0) == myhero.getTile()
							&& myhero.getBombNumberCurrent() == BOMBNUMBER
							&& ai.moveHandler.isBombing
							&& ai.moveHandler.canRun) {
