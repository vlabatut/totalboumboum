 ...
 * of certain scores during a tournament. It includes various controls
 * allowing to hide/display players and switch to other scores.
 * 
 * @author Vincent Labatut
 */
public class TournamentEvolutionSubPanel extends ColumnsSubPanel implements MouseListener
{	/** Class id for serialization */
	 ...
	 */
	public TournamentEvolutionSubPanel(int width, int height)
...
//		updatePlot();
	...
	// TOURNAMENT		/////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////
	/** Tournament displayed in this panel */
	private AbstractTournament tournament = null;
	
	/**
	 * Changes the tournament displayed in
	 * this panel.
	 * 
	 * @param tournament
	 * 		New tournament displayed in this panel.
	 */
	 ...
	 * current parameters : tournament, colors, score.
	 */
	@SuppressWarnings("unchecked")
	private void updatePlot()
	{	if(tournament!=null)
		{	// init player data
			StatisticTournament statisticTournament = tournament.getStats();
			List<StatisticMatch> statisticMatches = statisticTournament.getStatisticMatches();
			List<String> ids = statisticTournament.getPlayersIds();
			...
			// setting data model
			for(int i=0;i<statisticMatches.size();i++)
			{	StatisticMatch statisticMatch = statisticMatches.get(i);
				List<String> matchIds = statisticMatch.getPlayersIds();
				if(selectedScore==null)
				{	float values[] = statisticMatch.getPoints();
					for(int j=0;j<values.length;j++)
					{	String id = matchIds.get(j);
						int index = series.get(id);
						DataTable dataTable = dataTables.get(index);
						float count = countsFloat.get(index);
						count = count + values[j];
						countsFloat.set(index,count);
						dataTable.add(i+1,count);
					}
				}
				else
				{	long values[] = statisticMatch.getScores(selectedScore);
					for(int j=0;j<values.length;j++)
					{	String id = matchIds.get(j);
						int index = series.get(id);
						DataTable dataTable = dataTables.get(index);
						long count = countsInt.get(index);
						count = count + values[j];
						countsInt.set(index,count);
						dataTable.add(i+1,count);
					}
				}
				
				// complete players who didn't play the match
				for(String id: ids)
			...
			String xLabel = GuiConfiguration.getMiscConfiguration().getLanguage().getText(GuiKeys.COMMON_EVOLUTION_LABEL_MATCH);
...
//			axisRendererX.setSetting(AxisRenderer.TICKS_CUSTOM, labels);
	        Map<Double, String> labels = new HashMap<Double, String>();
			String prefix = GuiConfiguration.getMiscConfiguration().getLanguage().getText(GuiKeys.COMMON_EVOLUTION_LABEL_MATCH_PREFIX);
			for(int i=0;i<statisticMatches.size();i++)
...
//		repaint();
	...
	/** Colors used by the players of this tournament */
