...
/**
 * Cette class calcule la distance entre notre agent et les murs destructible et
 * elle nous permet de dire si on est à coté d'un mur destructible ou pas.
 * 
...
public class DestWallDist extends AiCriterionBoolean<Agent> {
	...
	public Boolean processValue(AiTile tile) {
		...
		for (AiBlock destBlocks : ai.getZone().getDestructibleBlocks()) {
			ai.checkInterruption();
			distanceBloks.add(ai.getZone().getTileDistance(tile,
					destBlocks.getTile()));
		}
