...
{	
	...
	{	
		...
		try
		{
			if(ai.securityHandler.willBurn(ownTile) && (ai.getZone().getBombs().size()>5 || ai.securityHandler.getTimeLeft(ownTile)<1350 || (ai.securityHandler.isDangerAround() && ai.securityHandler.isEnemyReachable()) || (timeLeft-(remainingPath/ownSpeed)<1750)))
			{
				usingDijkstra = true;
	...
	{	
		...
		{	
			...
			if(!usingDijkstra)
			{
				if((ai.securityHandler.willBurn(nextTile) && !ai.securityHandler.willBurn(currentTile)) || (!nextTile.getFires().isEmpty()) || ai.securityHandler.absoluteDeath(nextTile))
				{
					result = Direction.NONE;
				}
				else
