...
public class PreferenceHandler extends AiPreferenceHandler<Agent> {
     ...
     *
     * @return result
     *              un arbre contenant les case séléctionnée.
     */
	@Override
		...
		while (!queueTile.isEmpty()) {
			...
			for (Direction direction : Direction.getPrimaryValues()) {
				...
				if (tmpTile.getNeighbor(direction).getBlocks().isEmpty()
						...
						&& !result.contains(tmpTile.getNeighbor(direction))
	 ...
	 */
	private static String CAT_NAME_1 = "CAT_BOMB";
	/**
     ...
     *              la case de départ.
     * @return result
     *              la catégorie de la case séléctionnée.
     */
	...
	protected AiCategory identifyCategory(AiTile tile) {
		...
		if (mode == AiMode.ATTACKING) {
			...
			for (AiHero ennemy : ai.getZone().getRemainingOpponents()) {
				ai.checkInterruption();
				int distance = ai.getZone().getTileDistance(currentTile,
						ennemy.getTile());
				if (ai.getZone().getBombs().size() > 0)
