...
{	
	...
	/**
	 * Construit la partie de destination du gestionnaire de mode pour l'agent passé en paramètre.
	 * 
	 * @return 	
	 * 		renvoie une case de destination
	 */
	@Override
	...
	/////////////////////////////////////////////////////////////////
	/**
	 * Construit la partie de chemin du gestionnaire de mode pour l'agent passé en paramètre.
	 * 
	 * @return 	
	 * 		renvoie une chemin 
	 */
	...
	/////////////////////////////////////////////////////////////////
	/**
	 * Construit la partie de direction du gestionnaire de mode pour l'agent passé en paramètre.
	 * 
	 * @return 	
	...
	{	ai.checkInterruption();
		...
		{	
			...
			if(ai.willBurn(nextTile) && !ai.willBurn(currentTile))
			{
				result = Direction.NONE;
			}
			else if(ai.getTimeLeft(nextTile) < 1000 && ai.getTimeLeft(currentTile) <1000)
			{
				boolean control = false;
				AiTile safestTile=currentTile;			
				double countdown = ai.getTimeLeft(currentTile);				
					for(AiTile canEscape : currentTile.getNeighbors())
					{			
						ai.checkInterruption();
						if(ai.getTimeLeft(canEscape) > countdown && canEscape.isCrossableBy(zone.getOwnHero()))
						{
							safestTile = canEscape;
							countdown = ai.getTimeLeft(canEscape);
							control = true;
						}
					}
				if(control)
				{
				result = zone.getDirection(currentTile, safestTile);
				}
				else
				{
			...
			else if(ai.getTimeLeft(nextTile) >=  ai.getTimeLeft(currentTile))
			{
				result = zone.getDirection(currentTile, nextTile);
			}
			else
