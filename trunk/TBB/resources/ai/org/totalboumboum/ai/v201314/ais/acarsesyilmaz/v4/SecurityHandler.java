...
public class SecurityHandler extends AiAbstractHandler<Agent>{
	...
	public boolean isBombingSafe()
	{
		...
		simZone.createBomb(ownSimTile,ownSimHero);		
	...
	public double[][] processTileCosts()
	{
		...
		for(AiTile tile : ai.getZone().getTiles())
		{
			...
			{
				if(isDeadEnd(tile,Direction.NONE))
				{
	...
	/**
	 * Description : 
	 * 		Cette méthode verifie si la case donnée en parametre en dans une corridor ou non
	 * 
	 * @param tile
	 *  	la case a traiter
	 *  
	 * @return
	 * 		Renvoi true si la case est dans une corridor et false sinon
	 */
	public boolean isPassage(AiTile tile)
	{
		ai.checkInterruption();
		boolean result = false;
		
		if(getFreeNeighbours(tile).size() == 2)
		{
			if(Direction.areOpposite(ai.getZone().getDirection(tile,getFreeNeighbours(tile).get(0)),ai.getZone().getDirection(tile,getFreeNeighbours(tile).get(1))))
			{
				result = true;
			}
		}		
	 ...
	 * 		Renvoie true si oui et false sinon.
	 */
	public boolean isDeadEnd(AiTile tile,Direction direction)
	{
		ai.checkInterruption();		
		
		
		if(direction == Direction.NONE && absoluteDeath(tile)) // en köşe kontrolü
		{
			return true;
		}			
		else if(direction != Direction.NONE && (getFreeNeighbours(tile).size()<2 || ifExistsAnyoneButUs(tile))) //dead end
		{
			return true;
		}		
		else if(direction == Direction.NONE && getFreeNeighbours(tile).size()==1)
		{
			return isDeadEnd(getFreeNeighbours(tile).get(0),ai.getZone().getDirection(tile, getFreeNeighbours(tile).get(0))); 
		}		
		else if(getFreeNeighbours(tile).size()==2)
		{				
			if(isPassage(tile))
			{				
				if(direction == Direction.NONE)
				{			
					return isDeadEnd(tile, ai.getZone().getDirection(tile, getFreeNeighbours(tile).get(0))) && isDeadEnd(tile, ai.getZone().getDirection(tile, getFreeNeighbours(tile).get(1)));
				}
				else
				{
					tile = tile.getNeighbor(direction);
					return isDeadEnd(tile, direction);
				}					
			}
			else
			{
