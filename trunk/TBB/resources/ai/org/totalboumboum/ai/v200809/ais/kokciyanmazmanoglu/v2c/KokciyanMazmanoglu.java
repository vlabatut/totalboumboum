...
public class KokciyanMazmanoglu extends ArtificialIntelligence {
	...
	public AiAction processAction() throws StopRequestException {
		...
		if (ownHero != null) {
			...
			// if(targetTile != null)
			// System.out.println(newmaxval + " " +
			// ActionMatrix[targetTile.getLine()][targetTile.getCol()]);

			if (isTimeToRun2()) {

				if (mission == Mission.RUN_RUN_RUN_FAR_AWAY
						&& targetTile != null && isSafe2(targetTile)) {

					// System.out.println(targetTile.getLine() +" " +
					// targetTile.getCol());
					tree = new Tree(targetTile.getLine(), targetTile.getCol(),
							this);
					...
					if (targetTile != null) {
						// System.out.println(targetTile.getLine() +" " +
						// targetTile.getCol());
						tree = new Tree(targetTile.getLine(),
								targetTile.getCol(), this);
					} else
				...
				if (targetTile == null
						|| newmaxval > ActionMatrix[targetTile.getLine()][targetTile
								.getCol()]
						|| mission == Mission.RUN_RUN_RUN_FAR_AWAY) {
				...
				if (count != 2 && !canIMove()) {
					...
					if (!isBlockBetween(currentTile, targetTile)
							&& distance(currentTile, targetTile) <= ownHero
									.getBombRange()) {
						if (!isTrap3(currentTile))
							result = new AiAction(AiActionName.DROP_BOMB);
					} else {
					...
					if (distance(currentTile, targetTile) <= ownHero
							.getBombRange()) {
						if (!isTrap3(currentTile))
							result = new AiAction(AiActionName.DROP_BOMB);
					} else {
	 ...
	 *             Description manquante !
	 */
	public AiAction algoAEtoile(AiTile startTile, AiTile targetTile)
			throws StopRequestException {
		checkInterruption();
		AiAction result = new AiAction(AiActionName.NONE);
		...
		while (!control && !qNode.isEmpty()) {
			...
			if ((nActual.getLine() == endNode.getLine())
					&& (nActual.getCol() == endNode.getCol())) {
				...
				if (!path.isEmpty()) {
					...
					// path.elementAt(i).getAction().getDirection().name());

					// }

					result = path.elementAt(0).getAction();
					this.nextTile = path.elementAt(0).getChild()
							.convertToTile();
				}
				control = true;
			} else {
		...
		/*
		 * System.out.println("Startnode : " + startNode.getCol() + " " +
		 * startNode.getLine()); System.out.println("Endnode : " +
		 * endNode.getCol() + " " + endNode.getLine());
	...
	public void getMatrixValues() throws StopRequestException {
		...
		for (int i = 0; i < zone.getHeight(); i++) {
			...
			for (int j = 0; j < zone.getWidth(); j++) {
				...
				if (i == zone.getOwnHero().getLine()
						&& j == zone.getOwnHero().getCol())
					FieldMatrix[i][j] = 0;
				...
				// FieldMatrix[i][j]=1.0;
					...
					FieldMatrix[i][j] = distance(getCurrentTile(),
							zone.getTile(i, j));
					// FieldMatrix[i][j]=1;
				}
	...
	public boolean isObstacle(AiTile tile) throws StopRequestException {
		checkInterruption();
		if (tile.getBlock() == null && tile.getBombs().size() == 0
	 ...
	 *             Description manquante !
	 */
	public int distance(AiTile source, AiTile target)
			throws StopRequestException {
		checkInterruption();
		int res = 0;

		res = Math.abs(source.getLine() - target.getLine())
				+ Math.abs(source.getCol() - target.getCol());
	 ...
	 *             Description manquante !
	 */
	public boolean isMovePossible(AiTile tile, Direction d)
			throws StopRequestException {
		checkInterruption();
		boolean result;
		...
		switch (d) {
		case UP:
			result = tile.getLine() > 0
					&& !isObstacle(tile.getLine() - 1, tile.getCol());
			break;
		case DOWN:
			result = tile.getLine() < (zone.getHeight() - 1)
					&& !isObstacle(tile.getLine() + 1, tile.getCol());
			break;
		case LEFT:
			result = tile.getCol() > 0
					&& !isObstacle(tile.getLine(), tile.getCol() - 1);
			break;
		case RIGHT:
			result = tile.getCol() < (zone.getWidth() - 1)
					&& !isObstacle(tile.getLine(), tile.getCol() + 1);
			break;
		default:
			result = false;
	 ...
	 *             Description manquante !
	 */
	public boolean isMovePossible(int line, int col, Direction d)
			throws StopRequestException {
		checkInterruption();
		AiTile tile = zone.getTile(line, col);
	 ...
	 *             Description manquante !
	 */
	public boolean isTrap(AiTile ai, AiTile bombTile)
			throws StopRequestException {
		checkInterruption();
		...
		if (iterBomb.hasNext()) {
			...
			int power = bombe.getRange();// s'il existe une bombe en ces
											// coordonnées on parle d'un désir
											// du défense (le personnage veut
											// s'enfuir par une bombe)
			result = false;
			if (ai.getCol() - bombTile.getCol() > 0) {// si la bombe est à
														// gauche
				int i = 1;
				// s'il est impossible de faire un mouvement vers la droite,
				// vers le haut ou vers le bas c'est une piège
				if (!isMovePossible(ai, Direction.RIGHT)
						&& !isMovePossible(ai, Direction.UP)
						&& !isMovePossible(ai, Direction.DOWN)) {
					result = true;
				} else {
					while (i < power)// on va étudier une distance i partant 1
										// jusqu'à la portée de la bombe
					{
						checkInterruption();
						if (bombTile.getCol() + i < zone.getWidth())// si le
																	// personnage+la
																	// portée de
																	// la bombe
																	// est dans
																	...
																	// du jeu
						{
							if (possibleMoveD(ai.getLine(), ai.getCol(), i, 1))// s'il
																				// est
																				// possible
																				// d'aller
																				// vers
																				...
																				// i
							{
								// s'il est impossible de trouver une place à se
								// cacher en se déplacant pour une distance i
								if (!isMovePossible(ai.getLine() + i,
										ai.getCol(), Direction.UP)
										&& !isMovePossible(ai.getLine() + i,
												ai.getCol(), Direction.DOWN)) {
									result = true;
							...
							} else// s'il est impossible d'aller vers la droite
									// pour la distance i, il sera impossible de
									// s'enfuir
							{
								result = true;
								break;
			...
			{
				int i = 1;
				// s'il est impossible de faire un mouvement vers la gauche,
				// vers le haut ou vers le bas c'est une piège
			...
			if (ai.getLine() - bombTile.getLine() > 0) {// si la bombe est en
														// haut
				int i = 1;
				// s'il est impossible de faire un mouvement vers la droite,
				// vers la gauche ou vers le bas c'est une piège
				if (!isMovePossible(ai.getLine(), ai.getCol(), Direction.LEFT)
						&& !isMovePossible(ai.getLine(), ai.getCol(),
								Direction.RIGHT)
						&& !isMovePossible(ai.getLine(), ai.getCol(),
					...
					while (i < power) {
						...
						if (bombTile.getLine() + i < zone.getHeight()) {
							if (possibleMoveD(ai.getLine(), ai.getCol(), i, -2)) {
								if (!isMovePossible(ai.getLine(), ai.getCol()
										+ i, Direction.LEFT)
										&& !isMovePossible(ai.getLine(),
												ai.getCol() + i,
												Direction.RIGHT)) {
									result = true;
								} else {
			...
			if (ai.getLine() - bombTile.getLine() < 0) {// si la bombe est en
				...
				int i = 1;
				// s'il est impossible de faire un mouvement vers la droite,
				// vers le haut ou vers la gauche c'est une piège
				if (!isMovePossible(ai.getLine(), ai.getCol(), Direction.LEFT)
						&& !isMovePossible(ai.getLine(), ai.getCol(),
								Direction.RIGHT)
						&& !isMovePossible(ai.getLine(), ai.getCol(),
			...
			int i = power + 1;
			// si la bombe est à gauche et le personnage peut aller vers la
			// droite pour une distance plus grande que la portée de la bombe
			// alors ce n'est pas une piège
			if (ai.getCol() - bombTile.getCol() > 0
					&& possibleMoveD(bombTile.getLine(), ai.getCol(), i, 1))
				result = false;
			// si la bombe est à droite et le personnage peut aller vers la
			// gauche pour une distance plus grande que la portée de la bombe
			// alors ce n'est pas une piège
			else if (ai.getCol() - bombTile.getCol() < 0
					&& possibleMoveD(bombTile.getLine(), ai.getCol(), i, -1))
				result = false;
			// si la bombe est en bas et le personnage peut aller vers le haut
			// pour une distance plus grande que la portée de la bombe alors ce
			// n'est pas une piège
			if (ai.getLine() - bombTile.getLine() < 0
					&& possibleMoveD(ai.getLine(), bombTile.getCol(), i, 2))
				result = false;
			// si la bombe est en haut et le personnage peut aller vers le bas
			// pour une distance plus grande que la portée de la bombe alors ce
			// n'est pas une piège
	 ...
	 *             Description manquante !
	 */
	public boolean possibleMoveD(int x, int y, int d, int s)
			throws StopRequestException {
		checkInterruption();
		boolean result = false;
		switch (s) {
	...
	public int[] getClosestPlayerPosition() throws StopRequestException {
		...
		for (int i = 0; i < zone.getHeroes().size(); i++) {
			...
			int temp = distance(zone.getTile(zone.getOwnHero().getLine(), zone
					.getOwnHero().getCol()), zone.getTile(hero.getLine(),
					hero.getCol()));
			if (temp < minDistance) {
	 ...
	 *             Description manquante !
	 */
	public int[] getClosestBombPosition(AiTile tile)
			throws StopRequestException {
		checkInterruption();
		int minDistance = Integer.MAX_VALUE;
		int result[] = { -1, -1 };
	 ...
	 *             Description manquante !
	 */
	public boolean isSafe(AiTile bombeTile, AiTile me)
			throws StopRequestException {
		checkInterruption();
		boolean result = true;
		AiBomb bombe = bombeTile.getBombs().iterator().next();
		if ((bombeTile.getLine() == me.getLine() || bombeTile.getCol() == me
				.getCol()) && bombe.getRange() + 2 >= distance(bombeTile, me)) {
	...
	public boolean isSafe2(AiTile tile) throws StopRequestException {
		...
		if (a.hasNext()) {
			AiBomb bomb = a.next();
			if ((bomb.getLine() == tile.getLine() || bomb.getCol() == tile
					.getCol())
					&& ((bomb.getColor() != zone.getOwnHero().getColor() && bomb
							.getRange() > distance(bomb.getTile(), tile)) || ((bomb
							.getColor() == zone.getOwnHero().getColor() && zone
							.getOwnHero().getBombRange() > distance(
							bomb.getTile(), tile))))) {
	 ...
	 *             Description manquante !
	 */
	public boolean isBlockBetween(AiTile t1, AiTile t2)
			throws StopRequestException {
		checkInterruption();
		boolean result = false;
		if (t2.getLine() == t1.getLine()) {
			if (t2.getCol() < t1.getCol()) {
				...
				for (int i = 1; i < count; i++) {
					checkInterruption();
					if (isVoisinMur(
							zone.getTile(t2.getLine(), t2.getCol() + i),
							Direction.RIGHT)) {
						result = true;
						break;
					}
			...
			if (t2.getLine() < t1.getLine()) {
				...
				for (int i = 1; i < count; i++) {
					checkInterruption();
					if (isVoisinMur(
							zone.getTile(t2.getLine() + i, t2.getCol()),
							Direction.DOWN)) {
						result = true;
						break;
					}
	 ...
	 *             Description manquante !
	 */
	public boolean isVoisinMur(AiTile tile, Direction d)
			throws StopRequestException {
		checkInterruption();
		boolean res = false;
		Iterator<AiTile> iterTile = zone.getNeighborTiles(tile).iterator();
	 ...
	 *             Description manquante !
	 */
	public Vector<Direction> getPossibleMoves(int line, int col)
			throws StopRequestException {
		checkInterruption();
	 ...
	 *             Description manquante !
	 */
	public AiTile FindSafeCible2(AiTile bombe, AiTile me)
			throws StopRequestException {
		checkInterruption();
		...
		while (iterCase.hasNext()) {
			...
			if (i == 0 && isWall(t)) {
				// System.out.println(i+" "+getPercepts().getDirection(bombe,
				// t));
				if (possibleMoveD(me.getLine(), me.getCol(), 2, -2)) { // 2
																		// kadar
					// asagi
		...
		if (liste.size() != 0) {
			...
			possible.add(zone.getTile(me.getLine() - 2, me.getCol() - 1));// sol
																			// ust
																			// capraz
			possible.add(zone.getTile(me.getLine() - 2, me.getCol() + 1));// sag
																			// ust
																			// capraz
			possible.add(zone.getTile(me.getLine() + 2, me.getCol() - 1));// sol
																			// alt
	 ...
	 *             Description manquante !
	 */
	public List<AiTile> getClearNeighbors(AiTile tile)
			throws StopRequestException {
		checkInterruption(); // APPEL OBLIGATOIRE
	...
	public void decodeAction(AiTile tile) throws StopRequestException {
		...
		if (heroes2.size() > 0) {
			...
			if (tile.getItem() != null) {
				...
				if (item == AiItemType.EXTRA_BOMB
						&& zone.getOwnHero().getBombNumber() < 5)
					this.mission = Mission.GATHER_EXTRA_BOMB;
				else if (item == AiItemType.EXTRA_BOMB
						&& zone.getOwnHero().getBombNumber() == 5)
					this.mission = Mission.DESTROY_SURPLUS_BOMB;
				else if (item == AiItemType.EXTRA_FLAME
						&& zone.getOwnHero().getBombRange() < 5)
	 ...
	 *             Description manquante !
	 */
	public boolean FindSafeCible(AiTile bombe, AiTile me)
			throws StopRequestException {
		checkInterruption();
		Collection<AiTile> cases = getPercepts().getNeighborTiles(bombe);
		Iterator<AiTile> iterCase = cases.iterator();
		...
		if (liste.size() != 0) {
			/*
			 * Iterator<AiTile> iterListe = liste.iterator(); int mindistance =
			 * 1000; while (iterListe.hasNext()) { checkInterruption(); AiTile
			 * t2 = iterListe.next(); if (distance(t2, me) < mindistance) {
			 * mindistance = distance(t2, me); cible = t2; }
			 */
			synchronized (liste) {
		...
		if (me.getLine() + 2 < zone.getHeight() && 0 < me.getCol() - 1)
			possible.add(zone.getTile(me.getLine() + 2, me.getCol() - 1));// sol
																			// alt
																			// capraz
		if (me.getLine() + 2 < zone.getHeight()
				&& me.getCol() + 1 < zone.getWidth())
			possible.add(zone.getTile(me.getLine() + 2, me.getCol() + 1));// sag
																			// alt
																			// capraz
		...
		/*
		 * if(result.size()>0) cible= result.get(0); else result = null;
		 */
	...
	public boolean isTrap2(AiTile BombTile) throws StopRequestException {
		...
		for (int i = 0; i < zone.getHeight(); i++) {
			...
			for (int j = 0; j < zone.getWidth(); j++) {
				checkInterruption();
				if (BombTile.getLine() - range <= i
						&& i <= BombTile.getLine() + range
						&& BombTile.getCol() == j)
					matrix[i][j] = -1;
				else if (BombTile.getCol() - range <= j
						&& j <= BombTile.getCol() + range
						&& BombTile.getLine() == i)
					matrix[i][j] = -1;
				else {
	...
	public void updateAMWithTraps() throws StopRequestException {
		...
		for (int i = 0; i < zone.getHeight(); i++) {
			...
			for (int j = 0; j < zone.getWidth(); j++) {
				checkInterruption();
				if (zone.getTile(i, j).getItem() == null
						&& !isObstacle(zone.getTile(i, j))
						&& isTrap3(zone.getTile(i, j))) {
					ActionMatrix[i][j] = -9;
				}
