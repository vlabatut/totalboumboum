...
public class MoveHandler extends AiMoveHandler<AlcarKayaYildirim> {
	...
	/** */
	protected AiTile biggestTile = null;
	/** */
	...
	/** */
	protected ArrayList<AiTile> listTile = null;
	/** */
	protected AiTile lastTile = null;
	/** */
	protected AiTile tmpBiggest = null;
	/** */
	...
	protected boolean getSafeTile() throws StopRequestException {
		...
		// on ajoute tout les crossableby tiles a list
		for (int i = 0; i < 5; i++) {
			ai.checkInterruption();
		...
		while (it.hasNext()) {
			...
			try {
				pathControl = astarPrecise.startProcess(ownHeroLocation,firstTile);
			} catch (LimitReachedException e) {
				//e.printStackTrace();
			}
...
//		}
		...
		if (listTile.size() > 0) {
			lastTile = listTile.get(0);

			for (int i = 0; i < listTile.size(); i++) {
				this.ai.checkInterruption();
				if (lastTile == listTile.get(i)) {
					m++;
				}
		...
		if ( (m != 0) && (m == listTile.size()/2) && (loopCount <= 150) ) {
			loopCount++;
			return tmpBiggest;
