...
public class MoveHandler extends AiMoveHandler<OralOzugur> {
	 ...
	 */
	private static final int ATTACK_OBJECTIVE_DIFFERENCE = 0;
	/**
	 * Constructuer
	 * 
	 * @param ai
	...
	protected AiTile updateCurrentDestination() throws StopRequestException {
		...
		if (this.ai.getZone().getOwnHero().isContagious()) {
			...
			if(this.ai.nearest_enemy!=null){
				...
				return this.ai.objective=this.ai.nearest_enemy.getTile();
				}
...
//				if(!path.getLocation(NEXT_TILE).getTile().getItems().isEmpty())
//				{
//					for(AiItem item:path.getLocation(NEXT_TILE).getTile().getItems())
//					{
//						if(this.ai.isContagiousBonus(item)&&(!this.ai.contagiousBombed))
//						{
//							this.ai.contagiousTile=true;
//							return Direction.NONE;
//						}
//					}
//				}
//				this.ai.contagiousBombed=false;
