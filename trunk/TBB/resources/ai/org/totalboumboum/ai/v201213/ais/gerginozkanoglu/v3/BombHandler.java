		...
		else // attacking mode
		{
			if ( ourTile.equals( calculate.mostValuableTile() ) && !calculate.isDangerous(ourTile) && calculate.canReachSafety(this.ai.getZone().getOwnHero()))
			{
				...
				if(!enemyAccessible)
				{
					...
					if(this.ai.getZone().getBombsByColor(PredefinedColor.PINK).isEmpty() && !fireControl &&( calculate.numberOfDestructibleWalls(ourTile) > LIMIT_DEST_WALL || calculate.malusItemsCount(ourTile) > LIMIT_MALUS_COUNT)  && calculate.closestAndSafestTileAfterBombing(ourTile, this.ai.getZone().getOwnHero().getBombRange()) != null)
						return true;
					else
						return false;
				}
