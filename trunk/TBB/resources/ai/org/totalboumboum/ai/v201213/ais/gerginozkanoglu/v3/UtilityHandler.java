package org.totalboumboum.ai.v201213.ais.gerginozkanoglu.v3;
...
import org.totalboumboum.ai.v201213.ais.gerginozkanoglu.v3.criterion.ClosestWallToEnemy;
import org.totalboumboum.ai.v201213.ais.gerginozkanoglu.v3.criterion.Competition;
import org.totalboumboum.ai.v201213.ais.gerginozkanoglu.v3.criterion.NumberOfDestructibleWalls;
import org.totalboumboum.ai.v201213.ais.gerginozkanoglu.v3.criterion.Pertinence;
import org.totalboumboum.ai.v201213.ais.gerginozkanoglu.v3.criterion.Security;
import org.totalboumboum.ai.v201213.ais.gerginozkanoglu.v3.criterion.Time;
...
public class UtilityHandler extends AiUtilityHandler<GerginOzkanoglu> {
	...
	protected void initCases() throws StopRequestException {
		...
		criteria.add(criterionMap.get(Security.NAME));
		criteria.add(criterionMap.get(Time.NAME));
		criteria.add(criterionMap.get(ClosestWallToEnemy.NAME));
	...
	protected AiUtilityCase identifyCase(AiTile tile)
		...
		ClosestWallToEnemy closestWallToEnemy = (ClosestWallToEnemy) (criterionMap.get(ClosestWallToEnemy.NAME));
		TileCalculation calculate = new TileCalculation(this.ai);
...
//				//closest enemy
				...
				else if(!calculate.isThereEnemyInAllAccessibleTiles(this.ai.getZone().getOwnHero().getTile()))
				{
					...
					criterion.add(security);
					criterion.add(time);
					criterion.add(closestWallToEnemy);
	...
	protected void initReferenceUtilities() throws StopRequestException {
		...
		// on traite maintenant le mode attaque
		{
			...
			}
			{
				...
				combi.setCriterionValue((Time) criterionMap.get(Time.NAME),
						Boolean.FALSE);
				combi.setCriterionValue((ClosestWallToEnemy) criterionMap.get(ClosestWallToEnemy.NAME),
						Boolean.FALSE);
				defineUtilityValue(mode, combi, utility);
				utility++;
			}
