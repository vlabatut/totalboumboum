...
/**
 * This class is for expriming the zone of the game with a matrix placing
 * different danger levels to each case.
 * 
 * 
...
public class Safety_Map {
	...
	/** */
	private int pos_y;
	// Different exprimes for the cases possible. The more secured places will
	// be
	// exprimed with negative numbers and the dangerous cases will be exprimed
	// with positive ones.
	...
	public void Fill_The_Matrix() {
		...
		while (iterate_bombs.hasNext() == true) {
			...
			pos_x = bmb.getCol();
			pos_y = bmb.getLine();
			// calculating the time left before the explosion of the bomb and
			// placing this result(
			// we want to express the less time left with a bigger level so we
			// used:(1/timeleft)*100000)
			...
			// if not, we are making a countdown and calculating a danger level
			// for it:
			else {
			...
			if (blocks_left.isEmpty() == true && danger_level > 0) {
				for (int k = 1; k <= bmb.getRange(); k++) {
					if ((pos_x - k > 0)
							&& ((security_matrix[pos_y][pos_x - k] == SAFE_CASE) || (security_matrix[pos_y][pos_x
									- k] == BONUS))) {
						security_matrix[pos_y][pos_x - k] = danger_level;
					} else {
						if ((pos_x - k > 0)
