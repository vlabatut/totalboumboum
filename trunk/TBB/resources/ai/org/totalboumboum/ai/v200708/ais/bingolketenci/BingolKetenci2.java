...
//		carte = new int[17][15];
	...
	public Integer call() throws Exception {
		...
		if(carte==null)
			carte = new int[getZoneMatrixDimX()][getZoneMatrixDimY()];
		...
		// "move".
		for (int i = 0; i < getZoneMatrixDimX(); i++) { // sur l'axe x
			for (int j = 0; j < getZoneMatrixDimY(); j++) { // sur l'axe y

				if (i == 0 || i == getZoneMatrixDimX()-1 || j == 0 || j == getZoneMatrixDimY()-1)
...
//les cas où il ne peut pas se sauver
			...
			int dD = Math.max(getZoneMatrixDimX(),getZoneMatrixDimY());
	...
	private int[][] pointCase(int[][] tab) {
		...
		int[][] pCase = new int[getZoneMatrixDimX()][getZoneMatrixDimY()];
...
//on regarde si on peut poser une bombe pour tuer.On donne +20 points si c'st possible de poser une bombe.
	...
	public void oCarte(int[][] tab,int x,int y){
		if(tab[x][y]%10!=AI_BLOCK_WALL_HARD && tab[x][y]%10!=AI_BLOCK_WALL_SOFT && tab[x][y]%10!=AI_BLOCK_BOMB){
			carte[x][y] = tab[x][y];
			if(x+1 <= getZoneMatrixDimX()-1 && carte[x+1][y]== AI_BLOCK_UNKNOWN)
				oCarte(tab,x+1,y);
			if(x-1 >= 0 && carte[x-1][y]== AI_BLOCK_UNKNOWN)
				oCarte(tab,x-1,y);
			if(y+1 <= getZoneMatrixDimY()-1 && carte[x][y+1]== AI_BLOCK_UNKNOWN)
				oCarte(tab,x,y+1);
	...
	{	//à chaque appel de la fonction,on efface les anciens valeurs.
		...
		while((solution == null) && (!frange.isEmpty())){  
			...
			//la case qui est en bas de lui.
				if((temp.getY()+1<=getZoneMatrixDimY()-2)&& (tab[temp.getX()][temp.getY()+1]%10!=AI_BLOCK_FIRE) && (tab[temp.getX()][temp.getY()+1] >=0) && (tab[temp.getX()][temp.getY()+1]%10!=AI_BLOCK_BOMB)&&(tab[temp.getX()][temp.getY()+1]%10!=AI_BLOCK_WALL_HARD)&&(tab[temp.getX()][temp.getY()+1]%10!=AI_BLOCK_WALL_SOFT)&&(tab[temp.getX()][temp.getY()+1]!=AI_BLOCK_UNKNOWN)) {
