...
public class DeplacementCommune {
	 ...
	 * dire elle contr�le si la case est destructible ou indestructibles et il y
	 * a poss�de du danger ou pas . Elle prend 2 arguments la zone du jeu et une
	 * action.
	 * 
	 * @param gameZone
	 *            la zone du jeu
		...
		if (!endPoints2.isEmpty()) {
			...
			for (int i = 0; i < voisines.size(); i++) {
				...
				for (int j = 0; j < endPoints2.size(); j++) {
					...
					if (ai.print) {
						System.out.println("Direction de voisin:" + voisin
								+ dir);
						System.out.println("Direction d'endPoints:" + point
								+ dir2);
						System.out
								.println("---------------------------------------------------");
					}
	...
	public void nouvelleZone(int gauche, int droit, int haut, int bas,
		...
		int frontireHeight = height / 4;
		int frontiereWidth = width / 4;
