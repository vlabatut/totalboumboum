...
public class Path {
	 ...
	 *             description manquante !
	 */
	public Path(HacibektasogluIlgar hi, AiHero ourHero, AiHero adversaires[])
			throws StopRequestException {
	...
	/**
	 * on utilise cette method pour faire distinction de tiles qui sont deja
	 * calculé
	 * 
	 * @param tile
	 *            description manquante !
	 ...
	 *             description manquante !
	 */
	private boolean tileEstDejaAppartientAuPath(AiTile tile, AiPath path)
			throws StopRequestException {
		hi.checkInterruption();
	...
	/**
	 * cette methode sert à controller les tiles voisines pourque ne pas mettre
	 * une bombe si tous les tiles voisines sont <0
	 * 
	 * @param matriceTotal
	 *            description manquante !
	 ...
	 *             description manquante !
	 */
	public boolean voisineControl(double matriceTotal[][], AiPath tilesPossibles)
			throws StopRequestException {
		hi.checkInterruption();
		...
		for (int i = 0; i < voisines.size(); i++) {
			hi.checkInterruption();
			if (matriceTotal[voisines.get(i).getLine()][voisines.get(i)
					.getCol()] < 0)
				a++;
			else
	 ...
	 * il sert à voir si on mets une bombe est-ce qu'il reste aucune tile pour
	 * fuir
	 * 
	 * @param line
	 * 		description manquante !
	 ...
	 * 		description manquante !
	 */
	public void plusProcheAccesible(int line, int col, Etats[][] matriceEtat,
			double[][] matrice, int longueur, AiPath path)
			throws StopRequestException {
		hi.checkInterruption();
		...
		if (matrice[line][col] >= 0 && matriceEtat[line][col] != Etats.BONUS) {
			...
			if (!tileEstDejaAppartientAuPath(tile, path)
					&& !tile.equals(notreHeros.getTile())) {
				path.addTile(tile);
			}
			...
			plusProcheAccesible(mod(line + 1, height), col, matriceEtat,
					matrice, longueur - 1, path);
			plusProcheAccesible(line, mod(col - 1, width), matriceEtat,
					matrice, longueur - 1, path);
			plusProcheAccesible(line, mod(col + 1, width), matriceEtat,
					matrice, longueur - 1, path);
		}
	...
	/**
	 * on a essayé d'appliquer l'algorithme de largeur d'abord au debut le
	 * method prend une path qui a seulement le tile du hero et apres il regarde
	 * ses voisines et si le vosine est inpassable ou bien si il est deja
	 * utilisé on ne l'ajoute pas au path sinon on l'ajoute en regardent sa
	 * valeur de matriceTotale pour pouvoire decider si ça peut etre une cible
	 * ou pas.
	 * 
	 * @param matriceMarqueur
	 *            le matrice qui est utilisé pour comprendre si ce tile est deja
	 *            utilise ou pas il a comme valeurs : 0 : n'est pas utilise 1 :
	 *            utilisé et peut etre une cible -1 : utilisé mais ne peut pas
	 *            etre une cible
	 * 
	 * @param matriceTotal
	 *            le matrice qui est deja calcule d'apres la zone
	 * @param pilePath
	 *            le path qu'on va l'utiliser comme une pile
	 * @throws StopRequestException
	 * 		description manquante !
	 */
	public void cible(double matriceMarqueur[][], double matriceTotal[][],
			AiPath pilePath) throws StopRequestException {
		hi.checkInterruption();
		...
		for (int i = 0; i < voisines.size(); i++) {
			hi.checkInterruption();
			if (matriceMarqueur[voisines.get(i).getLine()][voisines.get(i)
					.getCol()] == 0
				...
				if (matriceTotal[voisines.get(i).getLine()][voisines.get(i)
					...
					matriceMarqueur[voisines.get(i).getLine()][voisines.get(i)
							.getCol()] = -1;
				} else
	 ...
	 * 		description manquante !
	 */
	public AiPath trouverChemin(int line, int col, double[][] matrice,
			AiTile tileCible) throws StopRequestException {
		hi.checkInterruption();
		...
		if (tileCible != null)
			try {
				pathAstar = astar.processShortestPath(zone.getTile(line, col),
						tileCible);
	...
	/**
	 * on calcule tous les tiles accesibles et apres on prend celui qui a
	 * meilleur note apres on donne la meilleur tile à Astar pourqu'il trouvé la
	 * meilleur route pour aller au cible à la fin on y bouge
	 * 
	 * @param line
	 * 		description manquante !
	 * @param col
	 ...
	 * 		description manquante !
	 */
	public AiPath trouverCible(int line, int col, double[][] matrice)
			throws StopRequestException {
		hi.checkInterruption();
		...
		if (matrice[tileCible.getLine()][tileCible.getCol()] == matrice[notreHeros
				.getLine()][notreHeros.getCol()])
			tileCible = notreHeros.getTile();

		// on control les 4 voisines tiles pour que l'ia ne bouge pas s'il y a
		// des bombes autour de lui

		// Maps map=new Maps(hi);

		AiPath tilesSecure = new AiPath();
		if (!voisineControl(matrice, tilesSecure)
				&& matrice[notreHeros.getLine()][notreHeros.getCol()] >= 0) {
			tileCible = notreHeros.getTile();
		}
	...
	/**
	 * Methode calculant la nouvelle action a effectuer (methode de groupe rouge
	 * de l'annee dernier mais on l'a modifié)
	 * 
	 * @param nextMove
	 ...
	 * 		description manquante !
	 */
	public AiAction nouvelleAction(AiPath nextMove, AiHero uneHero,
			double[][] matriceTotal) throws StopRequestException {
		hi.checkInterruption();
		...
		if (tiles.size() > 1) {
			...
			/*
			 * si le case suivant est un case dangeraux alors notre hero va
			 * attendre jusqu'il voit le danger se passer
			 */
			if (matriceTotal[notreHeros.getLine()][notreHeros.getCol()] >= 0
					&& matriceTotal[tiles.get(1).getLine()][tiles.get(1)
							.getCol()] < 0) {
				check = false;
			}
