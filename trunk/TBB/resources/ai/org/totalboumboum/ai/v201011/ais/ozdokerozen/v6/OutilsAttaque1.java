...
public class OutilsAttaque1 {
	 ...
	 *             description manquante !
	 */
	public OutilsAttaque1(OzdokerOzen a, AiZone b, int[][] c)
			throws StopRequestException {
		a.checkInterruption();
		AI = a;
		gameZone = b;
	...
	public AiTile bombBomMur(AiTile enemyTile) throws StopRequestException {
		...
		List<AiTile> scopes = new ArrayList<AiTile>();
		tileControleur.tilePossibleArriveAvecRisk(enemyTile,
				tilesPossibleAdversaire, scopes, matrice);
		tileControleur.tierLesTilesParDistanceEtAccebilite(
				tilesPossibleAdversaire, notreHero.getTile());
		...
		if (mursTiles.size() != 0) {
			if (tilesPossibleAdversaire.size() > 0) {
				tileControleur.tierLesTilesParDistanceEtAccebilite(mursTiles,
						tilesPossibleAdversaire.get(0));
				mur = mursTiles.get(0);
			}
		}
		return mur;
	...
	public void attackAlgorithm(AiTile adversaire, List<AiTile> cibles)
			throws StopRequestException {
		AI.checkInterruption();
		int calculerDistance = (int) tileControleur.calculerDistance(
				adversaire, notreHero.getTile());
		...
		if (calculerDistance < 1) {
			...
			// mais on ne peut pas poser bombe à otre tile, alors on aller une
			// autre tile possible
	 ...
	 *             description manquante !
	 */
	public boolean avoirSurUnBaril(AiTile adversaireTile, int special)
			throws StopRequestException {
		AI.checkInterruption();
		int coin = 0;
			...
			if (onDansRange(adversaireTile)) {
				for (int i = 0; i < voisin.size(); i++) {
					...
					if (matrice[voisin.get(i).getLine()][voisin.get(i).getCol()] == -1
							|| matrice[voisin.get(i).getLine()][voisin.get(i)
									.getCol()] == tileControleur.MURDESTRUCTIBLE
							|| matrice[voisin.get(i).getLine()][voisin.get(i)
									.getCol()] == tileControleur.MURINDESTRUCTIBLE)
						coin++;
				}
				if (coin >= 3) {
	 ...
	 *             description manquante !
	 */
	public boolean avoirSurUnCoridor(AiTile adversaireTile, int special)
			throws StopRequestException {
		AI.checkInterruption();
		int coin = 0;
		if (adversaireTile != null && special == 0) {
	...
	/**
	 * Methode qui retourne qu'il y a une obstacle entre notre hero et
	 * adversaire
	 * 
	 * @param adversaireTile
	 *            description manquante !
	 ...
	 *             description manquante !
	 */
	public boolean tilesEstPropre(AiTile adversaireTile, boolean line,
			boolean plus) throws StopRequestException {
		AI.checkInterruption();
		int i;
		boolean flag = true;
		...
		if (line) {
			if (plus) {
				for (i = adversaireTile.getCol() - 1; i > notreHero.getTile()
						.getCol(); i--) {
					AI.checkInterruption();
				...
				for (i = notreHero.getTile().getCol() - 1; i > adversaireTile
						.getCol(); i--) {
					AI.checkInterruption();
					if (matrice[adversaireTile.getLine()][i] == tileControleur.MURDESTRUCTIBLE
							|| matrice[adversaireTile.getLine()][i] == tileControleur.MURINDESTRUCTIBLE
			...
			if (plus) {
				for (i = adversaireTile.getLine() - 1; i > notreHero.getTile()
						.getLine(); i--) {
					AI.checkInterruption();
					if (matrice[adversaireTile.getLine()][i] == tileControleur.MURDESTRUCTIBLE
							|| matrice[adversaireTile.getLine()][i] == tileControleur.MURINDESTRUCTIBLE
				...
				for (i = notreHero.getTile().getLine() - 1; i > adversaireTile
						.getLine(); i--) {
					AI.checkInterruption();
