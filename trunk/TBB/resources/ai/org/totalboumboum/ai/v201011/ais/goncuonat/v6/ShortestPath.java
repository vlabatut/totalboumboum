...
public class ShortestPath {
	...
	/**
	 * 
	 * La methode pour remplir une liste des chemins possible pour avoir une
	 * occasion d'attaquer.
	 * 
	 * 
	 * @param endPoints
	 ...
	 * @throws StopRequestException
	 *             description manquante !
	 * @return result La liste des chemins pour un movement attaque
	 */
	public List<AiPath> shortestPathAttack(AiHero ownHero, AiTile startPoint,
			List<AiTile> endPoints) throws StopRequestException {
		monia.checkInterruption(); // APPEL OBLIGATOIRE
		// le chemin le plus court possible
		AiPath shortestPath = null;
		...
		try {
			for (int i = 0; i < endPoints.size(); i++) {
				monia.checkInterruption();
				shortestPath = astar.processShortestPath(startPoint,
						endPoints.get(i));
	 ...
	 * @throws StopRequestException
	 *             description manquante !
	 * @return AiPath le chemin entre 2 points donnees
	 * 
	 */

	public AiPath shortestPathEachTile(AiHero ownHero, AiTile startPoint,
	...
	public double distance(AiTile tile1, AiTile tile2)
			throws StopRequestException {
		monia.checkInterruption();
	...
	public AiHero getHero(AiTile hero, AiZone zone) throws StopRequestException {
		...
		if (heroes.size() != 0 && heroes != null) {
			for (int i = 0; i < heroes.size(); i++) {
				monia.checkInterruption();
				if (heroes.get(i).getLine() == hero.getLine()
						&& heroes.get(i).getCol() == hero.getCol())
					result = heroes.get(i);
			}
		}
		return result;
	...
	/**
	 * La methode qui permet de calculer liste des distance entre les endPoints
	 * et La case du rival plus proche.
	 * 
	 * @param closestEnemy
	 ...
	 * @throws StopRequestException
	 * 		description manquante !
	 * @return liste des distance entre les endPoints et La case du rival plus
	 *         proche
	 * 
	 */
	public List<Double> tileToClosestEnemy(AiTile closestEnemy,
			List<AiTile> endPoints, AiZone zone) throws StopRequestException {
		monia.checkInterruption();
