...
{	
	...
	{	// avant tout : test d'interruption
		...
		currentToleranceCofficient=120.0*selfHero.getTile().getSize()/selfHero.getWalkingSpeed();
	...
	public boolean isBombDropPermitted(Matrix m) throws StopRequestException
	{
		...
		if(!ret)
		{
			temporarilyIgnoredEmplacementCases.put(selfHero.getTile(),Long.valueOf(1250)); //500 //Ignoree pour 1.25 secondes
			if(m.regionEmplacementImportanceMatrix[selfHero.getLine()][selfHero.getCol()].forEnemy)
			{
	...
	private void changeMode() throws StopRequestException
	{
		...
		if(selfHero.getBombNumberMax()-selfHero.getBombNumberCurrent()+selfHero.getBombRange()>5 || (selfHero.getBombNumberMax()-selfHero.getBombNumberCurrent()) >=2 && selfHero.getBombRange() >=2) {
			if(mode==Mode.COLLECTE) {
