...
public class Maps 
{
	...
	AiItem bonuses[];
	static long bombExplotionVites=700;
			...
			if(zone.getHiddenItemsCount()>0)
			{
				if(matriceCollecte[heroLine][heroColonne]==Etats.VIDE_ET_MUR_DESTRUCTIBLE)
				{
					if(ourHero.getBombNumberMax()-ourHero.getBombNumberCurrent()>1)
					{
						...
						path.plusProcheAccesible(heroLine, heroColonne, matriceImaginaire, longueur,chemin);
					
						boolean control=path.voisineControl(matriceTotal, chemin);
						...
						else matriceTotal[ourHero.getLine()][ourHero.getLine()]=0;
	...
	private AiAction decisionBombPourAttaque(double matrice[][],AiPath chemin) throws StopRequestException, LimitReachedException
	{
		...
		if( matriceTotal[heroLine][heroColonne]>0)
		{
			if( matriceCollecte[heroLine][heroColonne]==Etats.VIDE_PRIORITAIRE ||
					matriceCollecte[heroLine][heroColonne]==Etats.VIDE_ET_MUR_DESTRUCTIBLE
