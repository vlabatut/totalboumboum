...
public class Path {
	 ...
	 * @throws StopRequestException
	 */
	public boolean voisineControl(double matriceTotal[][]) throws StopRequestException
	{
		notreIA.checkInterruption();
 ...
 */
	public void plusProcheAccesible(int line, int col,Etats[][] matriceEtat,double[][] matrice, int longueur,AiPath path) throws StopRequestException
	{
		...
		if(matrice[line][col]>=0 && matriceEtat[line][col]!=Etats.BONUS )
		{
			...
			{
				path.addTile(tile);
				AiOutput deneme=new AiOutput(zone);
				Color color=new Color(100, 100, 100);
				deneme.setTileColor(tile, color);
			}
			
		}
		else if(longueur==0 || matrice[line][col]==-100 || !zone.getTile(line, col).isCrossableBy(ourHero))
		{}
		else 
		{
	...
	public AiPath trouverCible(int line, int col,double[][] matrice) throws StopRequestException, LimitReachedException
	{
		...
		if(matrice[ourHero.getLine()][ourHero.getCol()]<-50)
			matriceMarqueur[ourHero.getLine()][ourHero.getCol()]=-1;
		else matriceMarqueur[ourHero.getLine()][ourHero.getCol()]=1;
...
//			pathAstar=new AiPath();
	 ...
	 * @throws StopRequestException
	 */
	public AiAction nouvelleAction(AiPath nextMove,AiHero uneHero) throws StopRequestException {
		notreIA.checkInterruption();
