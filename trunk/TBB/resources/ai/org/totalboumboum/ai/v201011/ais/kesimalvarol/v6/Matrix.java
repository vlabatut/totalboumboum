...
public class Matrix implements Cloneable {
	...
	/**
	 * Une deuxieme matrice a utiliser lors de prediction d'emplacement d'une
	 * nouvelle bombe
	 */
	public double representationBackupForBombPlacement[][]; // emplacement.
	/**
	 * Matrice a utiliser si on aura besoin de ajouter les parametres de
	 * distance aux cases neutres (valeur=0)
	 */
	public double representationDistanceParameters[][]; // distance for
														// neutrals.
	...
	public static class regionEmplacementImportance {
		 ...
		 *             description manquante !
		 */
		public regionEmplacementImportance(int i, boolean f)
				throws StopRequestException {
			monIA.checkInterruption();
			importance = i;
			forEnemy = f;
	...
	/**
	 * Sauvegarde une copie de matrice courant pour qu'on peut faire des
	 * predictions sur la posage de bombe
	 * 
	 * @throws StopRequestException
	 *             description manquante !
	 */
	 ...
	 * 
	 * @param azone
	 *            La zone du jeu dont les dimensions seront memorisees
	 * @throws StopRequestException
	 *             description manquante !
	 */
	public Matrix(AiZone azone) throws StopRequestException {
		...
		for (int i = 0; i < height; i++) {
			...
			for (int j = 0; j < width; j++) {
				monIA.checkInterruption();
				regionEmplacementImportanceMatrix[i][j] = new regionEmplacementImportance(
						0, false);
			}
		}
	 ...
	 * 
	 * @param targetBlock
	 *            Le mur sur lequel le calcul sera basee
	 * @param l
	 *            Le ligne courante
	 * @param c
	 *            Le colonne courante
	 * @param val
	 *            Le valeur a affecter
	 * @throws StopRequestException
	 *             description manquante !
	 */
	public void markForBombPlacementCandidate(AiBlock targetBlock, int l,
			int c, double val) throws StopRequestException {
		monIA.checkInterruption();

		if (targetBlock.getLine() < height && targetBlock.getCol() < width
				&& targetBlock.getCol() >= 0 && targetBlock.getLine() >= 0) {
			...
			do {
				...
				do {
					...
					position[1] = c + y;
					try {
						AiTile targtile = monIA.getZone().getTile(position[0],
								position[1]);
						if (targtile.getBlocks().size() > 0
								|| targtile.getItems().size() > 0)
							return;
						bombRegionNodes.add(monIA.getZone().getTile(
	 ...
	 *             description manquante !
	 */
	public void markForBombPlacementCandidate(AiHero targetHero,
			AiTile neighboringCase, double val) throws StopRequestException // ,int
																			// l,int
																			// c,double
																			// val)
																			// throws
																			// StopRequestException
	{
		...
		Direction propogationDirection = monIA.getZone().getDirection(target,
				neighboringCase);
		int portee = monIA.getSelfHero().getBombRange(), i = 0;
		while (portee - i > 0 && target.getBlocks().size() == 0
		...
		if (this.representation[neighboringCase.getLine()][neighboringCase
				.getCol()] >= 0 && neighboringCase.getBlocks().size() == 0)
			this.representation[neighboringCase.getLine()][neighboringCase
					.getCol()] += val;

		if (this.representation[target.getLine()][target.getCol()] >= 0
				&& target.getBlocks().size() == 0)
			this.representation[target.getLine()][target.getCol()] += (val * 1.5); // Les
																					// extremités
																					// des
																					// endroits
																					// ont
	...
	public void bestDirectionOptimisation() throws StopRequestException {
		...
		for (AiTile t : bombRegionNodes) {
			monIA.checkInterruption();
			int rimx = regionEmplacementImportanceMatrix[t.getLine()][t
					.getCol()].importance;
			boolean shallAdd = true;
			if (rimx >= 1) {
				for (AiTile t2 : t.getNeighbors()) {
					monIA.checkInterruption();
					if (!regionEmplacementImportanceMatrix[t2.getLine()][t2
							.getCol()].forEnemy
							&& regionEmplacementImportanceMatrix[t2.getLine()][t2
									.getCol()].importance < rimx) {
						regionEmplacementImportanceMatrix[t2.getLine()][t2
								.getCol()].importance = 0;
					} else if (!regionEmplacementImportanceMatrix[t.getLine()][t
							.getCol()].forEnemy
							&& regionEmplacementImportanceMatrix[t2.getLine()][t2
									.getCol()].importance > rimx) {
						regionEmplacementImportanceMatrix[t.getLine()][t
								.getCol()].importance = 0;
						shallAdd = false;
						break;
					}
				}
				if (shallAdd && t.getBlocks().size() == 0
						&& t.isCrossableBy(monIA.getSelfHero())) {
					if (this.representation[t.getLine()][t.getCol()] >= 0) {
						representation[t.getLine()][t.getCol()] += regionEmplacementImportanceMatrix[t
								.getLine()][t.getCol()].importance * 3;
					}
					tmp.add(t);
				}
			}
