				...
				if (!result1.isEmpty()) {
					...
					for (AiItem item : zone.getItems()) {
						...
						try {
							...
							AiLocation startLocation = new AiLocation(
									item.getTile());
							map = dijkstra.startProcess(startLocation);
							Iterator<AiTile> itr = map.keySet().iterator();
							AiFire fire = ownHero.getBombPrototype()
									.getFirePrototype();
							while (itr.hasNext() && retour) {
								...
								for (AiTile tiles : tile1.getNeighbors()) {
									...
									if (!tiles.getBlocks().isEmpty()) {
										...
										if (tiles.getBlocks().get(0)
												.isDestructible()) {

											for (AiTile tile2 : tiles
													.getNeighbors()) {
												ai.checkInterruption();
												if (accessible
														.processValue(tile2)
														&& tile2.isCrossableBy(fire)) {
													result.add(tile2);
												}
											}
								...
								if (!tiles.getBlocks().isEmpty()) {
									...
									if (tiles.getBlocks().get(0)
										...
										for (AiTile tile2 : tiles
												.getNeighbors()) {
											ai.checkInterruption();
											if (accessible.processValue(tile2)
													&& tile2.isCrossableBy(fire)) {
												result.add(tile2);
											}
										}
