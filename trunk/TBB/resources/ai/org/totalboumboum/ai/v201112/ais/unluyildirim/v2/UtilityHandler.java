...
import org.totalboumboum.ai.v201112.ais.unluyildirim.v2.criterion.CriterionAccessible;
import org.totalboumboum.ai.v201112.ais.unluyildirim.v2.criterion.CriterionBesoin;
import org.totalboumboum.ai.v201112.ais.unluyildirim.v2.criterion.CriterionConcurrence;
import org.totalboumboum.ai.v201112.ais.unluyildirim.v2.criterion.CriterionFacile;
import org.totalboumboum.ai.v201112.ais.unluyildirim.v2.criterion.CriterionFaible;
import org.totalboumboum.ai.v201112.ais.unluyildirim.v2.criterion.CriterionMenace;
import org.totalboumboum.ai.v201112.ais.unluyildirim.v2.criterion.CriterionTemps;
import org.totalboumboum.ai.v201112.ais.unluyildirim.v2.criterion.CriterionVoisin;
	...
	protected Set<AiTile> selectTiles() throws StopRequestException {
										...
										// choisir soit les items soit les murs
		{
			...
			// choisir les murs
			{
				...
				try {
					HashMap<AiTile, AiSearchNode> map = dijkstra
							.startProcess(startLocation);// essaie de trouver
															// tous les case
															// possbile d'aller
			...
			for (AiHero hero : zone_hero)// pour chaque adversaire dans la zone
			{
				...
				for (Direction d : Direction.getPrimaryValues()) {
					...
					while (i <= range && !retour) {
						...
						if (!accessible.processValue(voisin)
								&& voisin.isCrossableBy(fire)) {
							ai.checkInterruption();
		...
		if (result2.isEmpty()) {
			...
			if (result2.size() == result.size() || zone.getItems().isEmpty()) {
