package tournament200910.demirciduzokergok.v5_2;
...
/*
 * we declared the global variable to seperate the actions exploring the walls when the enemy is
 * not accessible and accessing to the enemie
 */
...
/* 
 *The classes for defense,collecting bonus,exploring walls and ,attacking the enemies
 */
 ...
 { 
  checkInterruption(); //APPEL OBLIGATOIRE
	  ...
	  if(our_bomberman.hasEnded()==false){
		...
		/*
		 * if we are in safe case and if we dont have bonus enough for attacking the enemie
		 * we collect the bonuses:
		 * For us,to be enough armored to attack an enemie is to have 3 bombs.
		 */
		else if(safe_map.returnMatrix()[our_bomberman.getLine()][our_bomberman.getCol()]!=safe_map.SAFE_CASE &&safe_map.returnMatrix()[our_bomberman.getLine()][our_bomberman.getCol()]!=safe_map.ENEMIE )
		{
			...
			escapemanager = new Escape_Manager(this);
			 ...
			 // System.out.println("bonus");
			  ...
			  /*
			   * The bonus exists but it is in range of fire or the path which we will cross
			   * for it is in range of bombs. It is dangerous to try to access for it, so
			   * we break walls to find new bonus which are not dangerous.
			   */
			  if(r==true && q_test==1){
				  bonusmanager=null;
				  if(wallmanager!=null){
			 ...
			 /*
			  * If no bonus exists in the zone(bonuses which are visible) we break walls to find
			  * bonus
			  */
		 ...
		 */
			  ...
			  if(enemiemanager!=null){
				  ...
				  if(enemiemanager.accessiblePath()==false  && kkk!=1){
					 ...
					 //This Method is for breaking walls when the enemie is not accessible
