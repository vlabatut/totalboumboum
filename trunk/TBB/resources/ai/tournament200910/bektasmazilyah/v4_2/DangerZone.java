...
//	System.out.println(result.toString());
	...
	{	source.checkInterruption(); //APPEL OBLIGATOIRE
		...
		{	source.checkInterruption(); //APPEL OBLIGATOIRE
			...
			{	source.checkInterruption(); //APPEL OBLIGATOIRE
				AiTile tile;
				AiTile temp = map.getTile(line,col);
				AiTile temp1= temp.getNeighbor(Direction.UP);
				if(getValeur(temp1.getCol(),temp1.getLine())== EtatEnum.BOMBE)
					up=false;
				...
				if(getValeur(temp1.getCol(),temp1.getLine())== EtatEnum.BOMBE)
					left = false;
				if(source.isDesctructible(col, line) && up && down && right && left)
					{
						tile=map.getTile(line, col);
						result.add(tile);
					}
...
//	System.out.println(result.toString());
	...
	public void afficheDestructibles(ArrayList<AiTile> destructibles) throws StopRequestException
	{
		source.checkInterruption();
