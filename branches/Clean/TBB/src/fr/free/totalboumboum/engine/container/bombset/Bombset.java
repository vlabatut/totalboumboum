	...
	{	Bomb result = null;
		...
		{	BombFactory bombFactory = it.next();
			...
			{	result = bombFactory.makeSprite(tile);
				result.setFlameRange(flameRange); // this is performed in BombsetManager.dropBomb() for the Heroes. Maybe should it be normalized ? use an ability ? (cf note in BombsetMgr)
			}
		}
		return result;
	}
