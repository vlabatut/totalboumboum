...
/*	public ActionAbility computeAbility()
	{	// actor original ability 
		ActionAbility result = actor.getAbility(this); // écrire getAbility(action), les autres sont-ils utiles?
		result = (ActionAbility)result.copy();
		
		// actor modulation
		if(result.isActive())
		{	ActorModulation actorModulation = actor.getActorModulation(this);
			if(actorModulation!=null) // peut être que c'est plus simple de renvoyer systmétiquement une modulation, mais avec une puissance de 0?
				result = actorModulation.modulate(result); // écrire cette méthode aussi, qui renvoie une nouvelle ability		
		}
		
		// target modulation (if there's one!)
		if(result.isActive() && target!=null)// quand on cherche une modulation pour un sprite donné, ça dépend de son gesture courant. si pas de gesture, alors il renvoie null
