...
*/
	...
	{	AbstractAbility copy = ability.copy();
		...
		directAbilities.add(copy);	// pourquoi toutes ces copies? la question est: qu'est-ce qui est modifié exactement dans l'ability. p-e qu'il n'est pas nécessaire de copier l'action
...
//		updateAbilities(modulationAbilities);
	...
	{	ArrayList<AbstractAbility> itemAbilities = sprite.getItemsAbilities();
		...
		if(itemAbilities.contains(ability))
			...
			a = directAbilities.get(directAbilities.indexOf(ability)); // questionable
		if(a!=null)
			a.modifyUse(delta);
	}
