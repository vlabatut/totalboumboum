...
//	System.out.println("H>>"+gesture.getName());
		...
		{	currentAnime = new AnimeDirection();
			...
			currentAnime.add(as);
			// à supprimer si le chargement d'animations est finalisé de manière à éviter cette situation (= anime vide)
		}
		if(reinit)
		{	isTerminated = false;
			...
			{	forcedDurationCoeff = 1;
				...
				isTerminated = false; // was true, but should not : it's supposed to be over only when the forcedDuration is over
			}			
			// actual anime
			else
...
//		System.out.println();
		...
		{	invisible = false;
			...
			{	twinkleTime = twinkleTime + milliPeriod;
				...
				if(showDuration<=0)
				{	if(hideDuration<=0)
						hideDuration = 500; //  arbitrary value
				...
				//  all the previous processing stuff could be done once and for all, if we suppose these abilities don't change, and the same holds for blinking
