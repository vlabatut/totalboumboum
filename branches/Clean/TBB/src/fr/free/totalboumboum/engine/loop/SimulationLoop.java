...
/*	
	...
	{	List<Profile> profiles = round.getProfiles();
		...
		double stdev = 1; // would be better with the actual standard-deviation
		...
		{	List<Profile> list = metalist.get(i);
			...
			{	Profile profile = list.get(i);
				...
				// time
				long time = playerScores.get(Score.TIME); // should be generalized for other points systems
				if(groupTime<0)
				...
				// other scores
				playerScores.put(Score.CROWNS,0l);// temporary;
				playerScores.put(Score.PAINTINGS,0l);// temporary;
			}
		}
