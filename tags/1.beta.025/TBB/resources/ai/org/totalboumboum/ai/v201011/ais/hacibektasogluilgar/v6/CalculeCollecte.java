...
public class CalculeCollecte {
	 ...
	 *             description manquante !
	 */
	public CalculeCollecte(HacibektasogluIlgar hi, CalculeZone cz, Action act)
			throws StopRequestException {
	...
	/**
	 * Dans le mode collecte on calcule s'il est necessaire de poser une bomb ou
	 * pas
	 * 
	 * @param matriceZone
	 * 		description manquante !
	 ...
	 * 		description manquante !
	 */
	private AiAction decisionBombPourCollecte(Etats[][] matriceZone,
			double matriceTotal[][], AiPath chemin)
		...
		if (matriceTotal[heroLine][heroColonne] > 0) {
			...
			if (cz.zone.getHiddenItemsCount() > 0 || cz.nbBonus > 0) {
				...
				boolean control = path
						.voisineControl(matriceTotal, tilesSecure);

				if (control) {
					double[][] matriceImaginaire = new double[cz.hauteur][cz.largeur];
					int control2 = act.matricePourImaginaireBombe(heroLine,
							heroColonne, matriceTotal, matriceImaginaire);

					if (control2 != -1) {
						int longueur = (int) (cz.bombeExplosionVitesse / notreHeros
								.getWalkingSpeed());

						// on calcule tous les tiles qui sont accesible dans le
						// cas de poser un bomb
						path.plusProcheAccesible(heroLine, heroColonne,
								matriceZone, matriceImaginaire, longueur + 2,
								chemin);

						if (chemin.getLength() > 0) {
							result = new AiAction(AiActionName.DROP_BOMB);
						} else {
							matriceTotal[notreHeros.getLine()][notreHeros
									.getCol()] /= 2;
							matriceTotal[notreHeros.getLine()][notreHeros
									.getCol()]--;
						}
	 ...
	 * 		description manquante !
	 */
	public AiAction calculeMatriceCollecte(Etats matrice[][],
		...
		for (int i = 0; i < cz.nbAdversaire; i++) {
			...
			act.calculeDapresPorte(cz.adversairesActuelles[i].getTile(),
					cz.matriceTotal, 20, ennemi);
		}
		...
		for (int i = 0; i < cz.hauteur; i++) {
			...
			for (int j = 0; j < cz.largeur; j++) {
				...
				// on donne couleurs aux tiles flammable vert, flammable rouge
				// et inaccesible
				...
				case MUR_DESTRUCTIBLE:
					...
					act.calculeDapresPorte(cz.zone.getTile(i, j), matriceTotal,
							10, mur);
					break;
				...
				case BONUS:
					/*
					 * on calcule tous les distances entre les heros et les
					 * bonus a la fin on calcule la matrice totale comme
					 * matriceTotal[i][j]+=100*(minDistance/distance1);
					 */
					int distance1 = Math.abs(notreHeros.getLine() - i)
							+ Math.abs(notreHeros.getCol() - j);
					int distance2[] = new int[cz.nbAdversaire];
					int minDistance = 100;
					for (int x = 0; x < cz.nbAdversaire; x++) {
						hi.checkInterruption();
						distance2[x] = Math.abs(cz.adversairesActuelles[x]
								.getLine() - i)
								+ Math.abs(cz.adversairesActuelles[x].getCol()
										- j);
						if (distance2[x] < minDistance)
							minDistance = distance2[x];
					}
				...
				// on donne le couleur vert aux tiles inaccesible en mode
				// collecte par notre héro
				if (matriceTotal[i][j] <= -100)
					output.setTileColor(i, j, case_inaccesible);
		...
		if (result == null) {
			AiPath pathCible = path.trouverCible(notreHeros.getLine(),
					notreHeros.getCol(), matriceTotal);
			...
			if (matriceTotal[notreHeros.getLine()][notreHeros.getCol()] <= -100) {
				if (path.voisineControl(matriceTotal, tilesSecure)
						&& tilesSecure != null) {
					pathCible = new AiPath();
					pathCible.addTile(notreHeros.getTile());
			...
			if (pathCible != null && pathCible.getLength() > 0)
				result = path.nouvelleAction(pathCible, notreHeros,
						matriceTotal);
		}
