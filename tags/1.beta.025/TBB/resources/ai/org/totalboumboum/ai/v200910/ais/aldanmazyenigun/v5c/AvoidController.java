...
/**
 * classe chargée d'implémenter un déplacement de fuite, (personnage menacé par
...
public class AvoidController {
	...
	/**
	 * crée un EscapeManager chargé d'amener le personnage au centre d'une case
	 * sûre
	 * 
	 * @param ai
	...
	public Direction update() throws StopRequestException {
		...
		if (!hasArrived()) { // on vérifie que le joueur est toujours sur le
			...
			if (path.getLength() > 1) {
				...
				if ((ai.getDangerLevel(path.getTile(0)) == 190 && ai
						.getDangerLevel(path.getTile(1)) <= 190)
						|| (ai.getDangerLevel(path.getTile(1)) < 11 && path
								.getLength() <= 7)
						|| (ai.getDangerLevel(path.getTile(0)) >= 11
								&& ai.getDangerLevel(path.getTile(0)) != 190 && ai
								.getDangerLevel(path.getTile(1)) <= ai
								.getDangerLevel(path.getTile(0))))
					tile = path.getTile(1);
				// updatePath();
			}
			...
			if (tile != null)
				...
				while (stop == 0
						&& myPosition.getCol() + k < ai.getZone().getWidth()) {
					ai.checkInterruption();
					if (safetyZ.getCaseLevel(myPosition.getLine(),
							myPosition.getCol() + k) == 9000
							|| safetyZ.getCaseLevel(myPosition.getLine(),
									myPosition.getCol() + k) == 90000
							|| safetyZ.getCaseLevel(myPosition.getLine(),
									myPosition.getCol() + k) == 91000
							|| safetyZ.getCaseLevel(myPosition.getLine(),
									myPosition.getCol() + k) >= 170)
						stop = 1;
					else {
						at.add(ai.getZone().getTile(myPosition.getLine(),
								myPosition.getCol() + k));
					}
					k++;
				}
				...
				stop = 0;
				while (stop == 0
						&& myPosition.getLine() + k < ai.getZone().getHeight()) {
					ai.checkInterruption();
					if (safetyZ.getCaseLevel(myPosition.getLine() + k,
							myPosition.getCol()) == 9000
							|| safetyZ.getCaseLevel(myPosition.getLine() + k,
									myPosition.getCol()) == 90000
							|| safetyZ.getCaseLevel(myPosition.getLine() + k,
									myPosition.getCol()) == 91000
							|| safetyZ.getCaseLevel(myPosition.getLine() + k,
									myPosition.getCol()) >= 170)
						stop = 1;
					else {
						at.add(ai.getZone().getTile(myPosition.getLine() + k,
								myPosition.getCol()));
					}
					k++;
				}
				...
				if (at != null) {
					...
					if (!at.isEmpty())
						min = ai.getDangerLevel(at.get(0));
					...
					result = ai.getZone().getDirection(ai.getOwnHero(),
							BestTile);
					updatePath();
