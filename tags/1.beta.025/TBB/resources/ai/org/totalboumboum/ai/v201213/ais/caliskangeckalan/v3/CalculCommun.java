			...
			while(it.hasNext() && !result) {
				...
				try {
					...
					if(tempPath != null) {
						...
						if(timeRemaining>crossTime) //control of can we escape
							result = true;
					}
				}
		...
		while(tileIt.hasNext()) {
			...
			for(int i = 0; i < items.size(); i++) {
				...
				if(!item.getType().equals(AiItemType.ANTI_BOMB) && !item.getType().equals(AiItemType.ANTI_FLAME) &&  !item.getType().equals(AiItemType.ANTI_SPEED) ) {
					...
					if(deathTiles == null || !deathTiles.contains(itemTile))
						result.add(itemTile);						
				}
			}
		}
	...
	public Set<AiTile> getMursNeighBoorsForCollecte(Map<AiTile,AiSearchNode> map, Set<AiTile> deathTiles) throws StopRequestException {
		...
		while(tileIt.hasNext()) {
			...
			if(tileNext.getNeighbor(Direction.RIGHT).getBlocks().size()> 0 && tileNext.getNeighbor(Direction.RIGHT).getBlocks().get(0).isDestructible()) {
				if(deathTiles == null || !deathTiles.contains(tileNext))
					result.add(tileNext);
			}
		...
		while(tileIt.hasNext()) {
			...
			for(int i = 0; i < heroes.size(); i++) {
				...
				if(!hero.equals(ownHero)) {
					...
					if(deathTiles == null || !deathTiles.contains(heroTile)) //control of the tile is mort subite 
						result.add(heroTile);
				}
			}
		}
		...
		while(tilesIt.hasNext()) {
			...
			while(heroesIt.hasNext()) {
			    ...
			    if(!hero.equals(ownHero)) {
			    	...
			    	print("distance:" +distance +"ownRange:" + ownRange);
