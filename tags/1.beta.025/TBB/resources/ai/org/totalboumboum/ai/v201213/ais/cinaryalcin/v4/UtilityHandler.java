...
import org.totalboumboum.ai.v201213.ais.cinaryalcin.v4.criterion.AdjacentTile;
import org.totalboumboum.ai.v201213.ais.cinaryalcin.v4.criterion.BlockEnemy;
import org.totalboumboum.ai.v201213.ais.cinaryalcin.v4.criterion.Concurrence;
import org.totalboumboum.ai.v201213.ais.cinaryalcin.v4.criterion.Distance;
import org.totalboumboum.ai.v201213.ais.cinaryalcin.v4.criterion.ItemType;
import org.totalboumboum.ai.v201213.ais.cinaryalcin.v4.criterion.Pertinent;
import org.totalboumboum.ai.v201213.ais.cinaryalcin.v4.criterion.Security;
...
{	
	...
	{	ai.checkInterruption();
		...
		else
		{
			...
			ai.ownZoneSearch=ai.getTilesReachable(ourhero);
			...
			//select sudden death cases in attack mode
			if(this.ai.isSuddenDeathClose(6000))
			{
				for(AiTile tilesudden:this.ai.getZone().getNextSuddenDeathEvent().getTiles())
				{
					ai.checkInterruption();
					if(!result.contains(tilesudden))
						result.add(tilesudden);
				}
			}
			...
			if(control)
			{
				...
				for(AiBomb bomb: this.ai.getZone().getBombs())
				{
					...
					{
						ai.checkInterruption();
						if(!result.contains(tile)&&tile.isCrossableBy(ourhero, false, false, false, false, true, true))
						result.add(tile);
					}
				}
				...
				{
					ai.checkInterruption();
					if(!result.contains(currentFire.getTile()))
					result.add( currentFire.getTile() );
				}
				...
				if(!this.ai.getZone().getRemainingOpponents().isEmpty())
				{
					AiTile tilesearchedfor=ourtile;
					AiHero nearesthero=ai.getNearestEnemy();
					AiTile tileenemy=nearesthero.getTile();
					boolean controldestructible=false;
					
					for (AiTile currentNeighbors : tilesearchedfor.getNeighbors()) {
						ai.checkInterruption();
						for (AiBlock currentblock : currentNeighbors.getBlocks()) {
							ai.checkInterruption();
							if (currentblock.isDestructible()) 
								controldestructible=true;	
						}
					}
					
					for(AiTile tile:ai.ownZoneSearch)
					{
						ai.checkInterruption();
						if(ai.getDist(tileenemy, tile)<ai.getDist(tileenemy, tilesearchedfor)||!controldestructible)
						{
							for (AiTile currentNeighbors : tile.getNeighbors()) {
								...
								for (AiBlock currentblock : currentNeighbors.getBlocks()) {
									ai.checkInterruption();
									if (currentblock.isDestructible()) 
										tilesearchedfor=tile;	
								}
							}
						}	
					}
					if(!result.contains(tilesearchedfor))
					result.add(tilesearchedfor);
				...
				for(AiItem item:tile.getItems())
				{
					...
					if (ai.Bonus.contains(item.getType())||
							ai.SBonus.contains(item.getType())
								||ai.Malus.contains(item.getType()))
						//Item visible
	...
	{	ai.checkInterruption();
		...
		{	mode = AiMode.COLLECTING;
			...
			combi.setCriterionValue((Distance)criterionMap.get(Distance.NAME),1);
			combi.setCriterionValue((Pertinent)criterionMap.get(Pertinent.NAME),Boolean.TRUE);
			defineUtilityValue(mode, combi, -300);
		}
