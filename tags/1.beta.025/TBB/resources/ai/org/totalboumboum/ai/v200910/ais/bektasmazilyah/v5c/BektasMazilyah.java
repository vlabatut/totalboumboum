...
public class BektasMazilyah extends ArtificialIntelligence {
	...
	@Override
	public AiAction processAction() throws StopRequestException { // Appel
																	// Obligatoire
		checkInterruption();
		...
		// si ownHero est null, ça veut dire l'IA est morte : inutile de
		// continuer
		if (hero != null) {
			updateLocation(hero.getTile());
			// on controlle si'il y a danger
			checkDanger();
			if (isDanger) {// Il y a un danger, il faut se sauver.
							// on precise la tile prochaine en utilisant la
							// methode suivre
				suivre();
				if (nextTile != null) {
			...
			// Il n'y a aucun danger
				...
				// S'il y a une adversaire proche a nous et si on peut deposer
				// une bombe, on la depose
				dropBomb = canDrop();
				if (isRivalInRange() && dropBomb) {
					...
					// Si on n'est pas assez armé et s'ils existent des bonus
					// ouvert, on part pour les prendre
					mustTakeBonus();
					if (bonusExiste() && takeBonus) {
						...
						canGoToRival();
						// Si on ne peut pas acceder a une enemie, on commance a
						// detruir des murs
						if (destructExiste() && !canGoRival) {
							...
							detruir();
							if (nextTile != null) {
								moveDir = zone.getDirection(currentTile,
										nextTile);
								dropBomb = canDrop();
								// si on est arrivé a targetTile et si on peut
								// deposer des bombes, on depose des bombes pour
								// detruir des murs
							...
							{
								// si on peut acceder a une ou plusieur
								// adversaires, on precise la tile prochaine
								// pour aller a adversaire
								goToRival();
								if (nextTile != null)
									moveDir = zone.getDirection(currentTile,
											nextTile);
	...
	private void goToRival() throws StopRequestException {
		...
		if (it.hasNext()) {
			checkInterruption();
			AiTile kare = it.next();
			as = new Astar(dangerZone, hero.getCol(), hero.getLine(),
					kare.getCol(), kare.getLine(), this);
			if (as.findSecurePath()) {
				...
				if (tile == hero.getTile()) {
					if (!deque.isEmpty()) {
						int tempx = deque.poll();
						int tempy = deque.poll();
						tile = zone.getTile(tempy, tempx);
					}
				}
			}
	...
	/*******************************************
	 * La méthode qu'on va utiliser pour connaitre s'il ya une adversaire proche
	 * a nous
	 * 
	 * @return si rival est proche a nous Description manquante !
	 * @throws StopRequestException
	 *             Description manquante !
	 */
	...
	/******************************************************************************
	 * La méthode qu'on va utiliser pour preciser la tile prochaine quand on
	 * suit
	 * 
	 * @throws StopRequestException
	 *             Description manquante !
	...
	/**********************************************************************************************
	 * La methode qu'on va utiliser pour connaitre si on peut deposer une bombe
	 * a ce moment la
	 * 
	...
	public boolean canDrop() throws StopRequestException {
		...
		if (result.size() > 0) {
			while (it.hasNext()) {
				...
				Astar a = new Astar(dangerZone, hero.getCol(), hero.getLine(),
						col, line, this);
				if ((hero.getCol() != col || hero.getLine() != line)
						&& a.findSecurePath())
					count++;
			}
		} else
		...
		// else
		/**
		 * { count=0; ArrayList<AiHero> heroes = new ArrayList<AiHero>();
		 * Iterator<AiHero> itHero = heroes.iterator(); while(itHero.hasNext()){
		 * temp=itHero.next().getTile(); Astar a=new Astar
		 * (dangerZone,hero.getCol
		 * (),hero.getLine(),temp.getCol(),temp.getLine()); if ((hero.getCol()!=
		 * temp.getCol() || hero.getLine() != temp.getLine()) && !a.findPath())
		 * count++; } if(count>0) dropBomb=true; }
		 */
		/*
		 * if(count==1) { count=0; temp=hero.getTile(); AiTile temp1=
		 * temp.getNeighbor(Direction.UP);
		 * if(dangerZone.getValeur(temp1.getCol(),temp1.getLine())==
		 * EtatEnum.BLOCDEST ||
		 * dangerZone.getValeur(temp1.getCol(),temp1.getLine())==
		 * EtatEnum.BLOCINDEST) count++; temp1=
		 ...
		 * EtatEnum.BLOCDEST ||
		 * dangerZone.getValeur(temp1.getCol(),temp1.getLine())==
		 * EtatEnum.BLOCINDEST) count++; if(count>=3) dropBomb=true; }
		 */
		return dropBomb;
	}

	/****************************************************************************************
	 * La methode qu'on va utiliser pour preciser les tiles qui se trouvent dans
	 * notre range
	 * 
	...
	private List<AiTile> preciserRange() throws StopRequestException {
		...
		while (k < range && up) {
			...
			if (etat != EtatEnum.BLOCDEST && etat != EtatEnum.BLOCINDEST
					&& etat != EtatEnum.BONUSBOMBE && etat != EtatEnum.BONUSFEU)
				result.add(temp);
			else
				up = false;
			k++;
	...
	private List<AiTile> removeFromList(List<AiTile> list)
			throws StopRequestException {
		checkInterruption();
	 ...
	 *             Description manquante !
	 */
	public List<AiTile> aligner(List<AiTile> source)
			throws StopRequestException {
		checkInterruption();
		...
		while (it.hasNext()) {
			checkInterruption();
			temp = it.next();
			Astar as = new Astar(dangerZone, hero.getCol(), hero.getLine(),
	 ...
	 * @param list
	 *            Description manquante !
	 * @return list Description manquante !
	 * @throws StopRequestException
	 *             Description manquante !
	 */
	public List<AiTile> findClearNeighbors(List<AiTile> list)
			throws StopRequestException {
		checkInterruption();
		List<AiTile> li = list;
	...
	/*********************************************************************************
	 * La methode qu'on va utiliser pour connaitre si on peut atteindre une
	 * adversaire
	 * 
	 * @throws StopRequestException
	 *             Description manquante !
	...
	private void canGoToRival() throws StopRequestException {
		...
		while (it.hasNext()) {
			checkInterruption();
			temp = it.next().getTile();
			Astar a = new Astar(dangerZone, getOwnHero().getCol(), getOwnHero()
					.getLine(), temp.getCol(), temp.getLine(), this);
			if ((hero.getCol() != temp.getCol() || hero.getLine() != temp
					.getLine()) && a.findSecurePath())
				canGoRival = true;
		}
	}
	 ...
	 *             Description manquante !
	 */
	public List<AiTile> pathTiles(Deque<Integer> deq)
			throws StopRequestException {
		checkInterruption();
		AiTile tile;
	...
	public boolean isClear(int x, int y) throws StopRequestException {
		...
		return (etat == EtatEnum.LIBRE || etat == EtatEnum.BONUSBOMBE
				|| etat == EtatEnum.BONUSFEU || etat == EtatEnum.HERO || etat == EtatEnum.RIVAL);
	}
	 ...
	 * 
	 * @return s'il y a des bonus ouvert Description manquante !
	 * @throws StopRequestException
	 *             Description manquante !
	 */
	 ...
	 * 
	 * @return zone Description manquante !
	 * @throws StopRequestException
	 *             Description manquante !
	 */
