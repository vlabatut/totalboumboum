...
public class MancuhanPinarer extends ArtificialIntelligence {
			...
			// L'attaque, pas encore implemente.
				...
				if (gameZone.getRemainingHeroes().size() > 1) {
					...
					if (isDead)
						chosenEnemy = null;
					if(!checkBomb){
						//System.out.println("Bombayi beklemiyecek");
						this.attackAlgorithm(gameZone, matrice);
					}
					else{
						//System.out.println((openBomb.getNormalDuration()+openBomb.getExplosionDuration())-openBomb.getTime());
	...
	private void attackAlgorithm(AiZone gameZone, int[][] matrice)
		...
		if (nextMoveAttack == null) {
			if (chosenEnemy == null) {
				...
				System.out.println("Secilen Dusman" +chosenEnemy.getColor());
			}
			...
			if (!isAccessible) {
				...
				if (accessibleTiles.size() != 0) {
					...
					this.nextMoveAttack = this.calculateShortestBonusPath(
				...
				this.nextMoveAttack = this.calculateShortestBonusPath(ourHero,
			...
			if (nextMoveAttack.getLength() == 0) {
				...
				if(openPath){
					...
					if(gameZone.getTileDistance(closestTile, chosenEnemy.getTile())<gameZone.getTileDistance(end, chosenEnemy.getTile()))
						adapt=true;
				}
	...
	private boolean isAccessibleEnemy(AiHero enemy) throws StopRequestException {
		...
		AiPath temp = this.calculateShortestBonusPath(ourHero, ourHero
				.getTile(), endPoint);
	...
	private AiHero accessibleEnemyExists(AiZone gameZone)
		...
		for (int i = 0; i < heros.size(); i++) {
			...
			AiPath tempPath = this.calculateShortestBonusPath(ourHero, ourHero
					.getTile(), tempTiles);
				...
				if (matrice[nextMove.getLastTile().getLine()][nextMove
					...
					if (adapt)
						...
						if ((this.ourHero.getLine() == this.nextMove.getTile(0)
								.getLine())	&& (this.ourHero.getCol() == this.nextMove.getTile(0).getCol())) {
