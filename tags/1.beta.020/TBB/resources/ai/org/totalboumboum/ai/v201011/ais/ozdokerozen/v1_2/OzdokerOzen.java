...
{	
	...
	/**
	 * Methode definissant le mur danslequel notre hero va poser bombe
	 * @param AiHero
	 * 			notre hero
 	 * @param gameZone
	 * 			l'aire de jeu
	...
	private AiTile yakinyerbul2(int[][] matrice,AiHero notreHero,AiZone gameZone) throws StopRequestException, LimitReachedException{
		...
		for(int i=0;i<matrice.length;i++){
			for(int j=0;j<matrice.length;j++){
				if(matrice[j][i]==1){
					if(outiles.existePath(gameZone, matrice, gameZone.getTile(j,i))){
						surePlace.add(gameZone.getTile(j,i));
					}
				}
			}
		}
