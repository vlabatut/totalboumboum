...
/**
 * La classe qui sert a modifier la zone du jeu, particulierement pour poser des bombes, en modifiant la zone comme on avait posee une bombe.
 * On pourrait bien utiliser AiModel, mais on avait déja implementé celle-ci et notre IA n'a pas vraiment besoin de toutes ses fonctionnalités servies.
 * 
...
public class GameZoneFuturePredictions
{
	 ...
	 */
	public static void modifyMatrixWithFutureBomb(Matrix m) throws StopRequestException
	{
		monIA.checkInterruption();
			...
			if(iC<monIA.getZone().getHeight() && iC>=0) {
				...
				try {
					if(zone.getTile(iC, j).getBlocks().size()==0)
					{
						...
						if(monIA.verbose) System.out.println(m.representation[iC][j]);
					}
					else break;
				}
			...
			if(jC<monIA.getZone().getWidth() && jC>=0) {
				...
				try {
					if(zone.getTile(i, jC).getBlocks().size()==0)
					{
						...
						if(monIA.verbose) System.out.println(m.representation[i][jC]);
					}
					else break;
				}
				catch(ArrayIndexOutOfBoundsException e)
	...
	{
		monIA.checkInterruption();
		return areBonusAdjacentAndNotTargetingEnemies(m, monIA.getSelfHero().getTile());
	}
	...
	{
		monIA.checkInterruption();
		int i=currentLocation.getLine(),j=currentLocation.getCol(),bombrange=monIA.getSelfHero().getBombRange();
		...
		//Si celle-ci est pour detruire les adversaires, on va laisser la bombe.
