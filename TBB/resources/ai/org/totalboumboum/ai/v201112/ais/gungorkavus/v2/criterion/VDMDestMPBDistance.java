...
*/
public VDMDestMPBDistance(GungorKavus ai) throws StopRequestException
{	// init nom
super(NAME);
...
{	
	...
	if(blockL.size()>0){
		...
		for(int i = 0;i<blockL.size()-1;i++){
			...
			if(zone.getTiles().contains(blockL.get(i+1).getTile().getNeighbor(Direction.UP))){
			...
			if(zone.getTiles().contains(blockL.get(i+1).getTile().getNeighbor(Direction.LEFT))){
			...
			if(zone.getTiles().contains(blockL.get(i+1).getTile().getNeighbor(Direction.RIGHT))){
