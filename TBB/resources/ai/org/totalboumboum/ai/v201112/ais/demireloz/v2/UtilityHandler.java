...
import org.totalboumboum.ai.v201112.ais.demireloz.v2.criterion.Competition;
import org.totalboumboum.ai.v201112.ais.demireloz.v2.criterion.Convenience;
import org.totalboumboum.ai.v201112.ais.demireloz.v2.criterion.Danger;
import org.totalboumboum.ai.v201112.ais.demireloz.v2.criterion.Distance;
import org.totalboumboum.ai.v201112.ais.demireloz.v2.criterion.IsEnemyReachable;
import org.totalboumboum.ai.v201112.ais.demireloz.v2.criterion.NbrAdjacentWall;
import org.totalboumboum.ai.v201112.ais.demireloz.v2.criterion.Time;
...
public class UtilityHandler extends AiUtilityHandler<DemirelOz> {
	...
	protected void initCriteria() throws StopRequestException {
		...
		{
			combi1 = new AiUtilityCombination(case8);
			combi1.setCriterionValue(isenemyreachable, 4);
			combi1.setCriterionValue(time, true);
		...
		}
		{
			...
			combi1.setCriterionValue(distance, true);
			combi1.setCriterionValue(danger, true);
		...
		{
			combi1 = new AiUtilityCombination(case6);
			combi1.setCriterionValue(distance, false);
			combi1.setCriterionValue(danger, true);
			combi1.setCriterionValue(nbrAdjacentWall, 3);
	...
	protected AiUtilityCase identifyCase(AiTile tile)
		...
		// SelectTiles.
		for (AiTile aitile : selectTiles()) {
			ai.checkInterruption();
			if (!aitile.getHeroes().isEmpty()
					&& !aitile.getHeroes().contains(ourhero)) {
		...
		if (mode == AiMode.ATTACKING) {
			...
			for (Direction direction : Direction.getPrimaryValues()) {
				...
				if (herocontrol) {
					if (!tile.getItems().isEmpty() || tile.getItems().isEmpty()) {
						for (AiBlock currentBlock : tile.getNeighbor(direction)
								.getBlocks()) {
							ai.checkInterruption();
							if (currentBlock.isDestructible()) {
								...
								criteria.add(isenemyreachable);
								criteria.add(time);
								return result = new AiUtilityCase(caseName8,
										criteria);
