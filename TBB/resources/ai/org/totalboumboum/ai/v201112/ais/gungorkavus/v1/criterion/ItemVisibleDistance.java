...
{	/** Nom de ce critère */
	...
	{	boolean result = false;
		...
		if(itemL.size()>0){
		double shortest = distance(itemL.get(0).getCol(), itemL.get(0).getRow(), ownHero.getCol(), ownHero.getRow()); ;
		...
		for(int i = 0;i<itemL.size()-1;i++)
		{
			...
			if(shortest > distance(itemL.get(i+1).getCol(), itemL.get(i+1).getRow(), ownHero.getCol(), ownHero.getRow()))
			{
				shortest =  distance(itemL.get(i+1).getCol(), itemL.get(i+1).getRow(), ownHero.getCol(), ownHero.getRow());
				sItem = itemL.get(i+1);
		...
		for(int i = 0;i<itemL.size()-1;i++)
		{
			...
			if(shortest == distance(itemL.get(i+1).getCol(), itemL.get(i+1).getRow(), ownHero.getCol(), ownHero.getRow()))
			{
				...
				if(distance(tile.getCol(), tile.getRow(),dItem.getCol(), dItem.getRow())==0)
				{
					result = true;
				}
			}
