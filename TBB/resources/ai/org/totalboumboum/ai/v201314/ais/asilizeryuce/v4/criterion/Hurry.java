 ...
 * l'ennemi, qu'on veut mettre une bombe. aussi elle contole apres la posage que
 * est-ce qu'on peut se proteger
 * 
 * @author Emre Asıl
 * @author Tülin İzer
 * @author Miray Yüce
...
public class Hurry extends AiCriterionInteger<Agent> {
	...
	AiTile bombTile;
	/**
	 * en posant une bombe dans cette case on bloque la flamme et on gange une
	 * possibilite de s'enfuire
	 */
	AiTile flammeTile;
	...
	public Integer processValue(AiTile tile) {
		...
		if (!done) {
			...
			for (AiTile tiles : ai.enemyHandler.enemyTiles()) {
				ai.checkInterruption();
				if (tiles.getCol() == hero.getCol())
					enemy = tiles;
			}
			if (ai.tileHandler.simpleTileDistance(enemy, hero.getTile()
					.getNeighbor(Direction.DOWN)) < ai.tileHandler
					.simpleTileDistance(enemy,
							hero.getTile().getNeighbor(Direction.UP)))
				bombTile = hero.getTile().getNeighbor(Direction.DOWN);
			else
				bombTile = hero.getTile().getNeighbor(Direction.UP);

			for (AiTile tiles : ai.enemyHandler.enemyTiles()) {
				ai.checkInterruption();
				if (tiles.getRow() == hero.getRow())
					enemy = tiles;
			}

			if (ai.tileHandler.simpleTileDistance(enemy, hero.getTile()
					.getNeighbor(Direction.LEFT)) > ai.tileHandler
					.simpleTileDistance(enemy,
							hero.getTile().getNeighbor(Direction.RIGHT)))
				flammeTile = hero.getTile().getNeighbor(Direction.LEFT);
			else
				flammeTile = hero.getTile().getNeighbor(Direction.RIGHT);

			done = true;
		}

		if (tile.equals(bombTile) && tile.getBombs().isEmpty())
			return 0;

		if (tile.equals(flammeTile) && tile.getBombs().isEmpty())
			return 1;
