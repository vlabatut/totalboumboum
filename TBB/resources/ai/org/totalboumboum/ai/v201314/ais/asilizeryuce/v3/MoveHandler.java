...
public class MoveHandler extends AiMoveHandler<Agent> {
	...
	protected MoveHandler(Agent ai) {
		...
		// this.astarApproximate.getCostCalculator().setOpponentCost(1000);
	...
	protected AiPath processCurrentPath() {
		...
		/*
		 * If none of the enemies are accessible use astar approximatif to find
		 * indirect path
		 */
		else {
			if (blockedTile != null) {
			...
			if (firstIndirectPath == false
					|| (controlSecurity && controlBomb && controlFire)) {
				// System.out.println("approximatif");
				try {
				...
				if (result != null) {
					...
					while (it.hasNext()) {
						...
						// tile da malus varsa bombayla patlatmak icin
			...
			/*
			 * We reached to the first part of the indirect path, now we use
			 * time based astar for the second part
			 */
			else {
				ArrayList<AiTile> dTiles = ai.tileHandler.dangerousTiles();
				for (AiTile tile : ai.tileHandler.accessibleTiles) {
					ai.checkInterruption();
	...
	protected Direction processCurrentDirection() {
		...
		/* Path is null or its only our tile */
