...
public class PreferenceHandler extends AiPreferenceHandler<Agent> {
	 ...
	 * Cette méthode concerne seulement les structures de données créées par le
	 * concepteur : les structures imposées sont réinitialisées par resetData().
	 * Cette méthode est appelée automatiquement, vous (le concepteur de
	 * l'agent) n'avez pas besoin de l'appeler.
	 */
	@Override
	...
	/**
	 * @param tile
	 *            Réinitialise les structures de données modifiées à chaque
	 *            itération. Cette méthode concerne seulement les structures de
	 *            données créées par le concepteur : les structures imposées
	 *            sont réinitialisées par resetData(). Cette méthode est appelée
	 *            automatiquement, vous (le concepteur de l'agent) n'avez pas
	 *            besoin de l'appeler.
	 * 
	 * @return category
	 */
	...
	protected AiCategory identifyCategory(AiTile tile) {
		ai.checkInterruption();
		AiCategory result = getCategory(null);
	...
	/**
	 * Met à jour les sorties graphiques de l'agent en considérant les données
	 * de ce gestionnaire. Ici, on affiche la valeur numérique de la préférence
	 * dans chaque case, et on colorie la case en fonction de cette valeur : la
	 * couleur dépend du mode (bleu pour la collecte, rouge pour l'attaque) et
	 * son intensité dépend de la préférence (clair pour une préférence faible,
	 * foncé pour une préférence élevée). Cette méthode peut être surchargée si
	 * vous voulez afficher les informations différemment, ou d'autres
	 * informations. A noter que cette méthode n'est pas appelée automatiquement
