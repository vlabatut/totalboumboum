...
{	/** Nom de ce critère */
	...
	{	ai.checkInterruption();
		...
		/*
		 *  Baktıgımız bütün caseların etrafındaki duvar sayısını sayıyoruz.. 
		 *  if in icindeki // kaldırırsan bombayıda bir duvar gibi sayabilir..
		 */
		List<AiTile> NList=tile.getNeighbors();
		for(int i=0;i<4;i++){
			ai.checkInterruption();
			if(!NList.get(i).getBlocks().isEmpty())// || NList.get(i).getBombs().isEmpty())
			{
				result++;
			}
		}
