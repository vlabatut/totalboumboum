...
public class PreferenceHandler extends AiPreferenceHandler<Agent>
{
    ...
    {
        /**
         * This method uses the "largeur d'abord" algorithm to find accessible
         * tiles considering the obstacles from our hero's tile.
         */
        ai.checkInterruption();
    ...
    protected AiCategory identifyCategory(AiTile tile)
    {
        ...
        AiTile ownTile = tile.getZone().getOwnHero().getTile();

        // Attacking Mode
        if(ai.modeHandler.getMode() == AiMode.ATTACKING)
        {
            if(getNearTiles(ownTile, 4).contains(tile))
            {
