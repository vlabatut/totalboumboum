...
public class MoveHandler extends AiMoveHandler<Agent> {
	...
	protected AiTile processCurrentDestination() {
		...
		int maxPref = Collections.max(preferences.keySet()); // ATTENTION : ici
																// il faudrait
																// tester qu'il
																// y a au moins
																// une valeur
																...
																// !)
		List<AiTile> tiles = preferences.get(maxPref); // on récupère la liste
														// de cases qui ont la
														// préférence maximale
	...
	protected AiPath processCurrentPath() {
		...
		try { // on doit OBLIGATOIREMENT exécuter astar dans un bloc try/catch
			result = astar.startProcess(startLocation, endTile);
											...
											// faut PAS afficher la trace (cf.
											// le manuel de l'API, la partie sur
											// les contraintes de programmation)
											// l'exception est levée seulement
											// si astar atteint certaines
											// limites avant de trouver un
											// chemin
											// cela ne veut donc pas dire qu'il
											// y a eu une erreur, ou que le
											// chemin n'existe pas.
											// ça veut seulement dire que ça
											// prend trop de temps/mémoire de
											// trouver ce chemin
											// dans ce cas là, il faudrait, dans
											// ce bloc catch, effectuer un
											// traitement spécial pour résoudre
											// ce problème. Ici, pour l'exemple,
											// on se contente de construire un
											// chemin vide (ce qui n'est pas
											// très malin)
			result = new AiPath();
		}
	...
	protected Direction processCurrentDirection() {
		...
		if (path == null || path.getLength() < 2) // cas où le chemin est vide,
			...
			result = zone.getDirection(currentTile, nextTile); // ici, j'utilise
																// une méthode
																// de l'API pour
																// calculer la
																// direction
