...
public class PathSafetyDeterminators {
	 ...
	 * 
	 * @param p
	 *            Le chemin a utiliser
	 * @param m
	 *            La matrice a utiliser
	 * @return Si le chemin n'est pas en surete
	 * @throws StopRequestException
	 *             description manquante !
	 */
	public static boolean isThisPathDangerous(AiPath p, Matrix m)
			throws StopRequestException {
		monIA.checkInterruption();
	 ...
	 *            La matrice a utiliser
	 * @param offset
	 *            Si on ne commence pas de la case 1er du chemin
	 * @return Si le chemin n'est pas en surete
	 * @throws StopRequestException
	 *             description manquante !
	 */
	public static boolean isThisPathDangerous(AiPath p, Matrix m, int offset)
			throws StopRequestException {
		monIA.checkInterruption();
		...
		if (tiles.size() > 0) {
			if (monIA.verbose)
				System.out.println("Path security check , start from "
						+ p.getTile(offset));
		}
		if (monIA.verbose)
			System.out.println("Am I in danger anyway ?"
					+ monIA.isInDangerAndWontMoveAnyway());
		for (int i = offset; i < tiles.size(); i++) {
			monIA.checkInterruption();
			AiTile t = p.getTile(i);
			if (stepsTaken >= 1
					&& m.representation[t.getLine()][t.getCol()]
							- (monIA.getCurrentToleranceCofficient() / monIA
									.getUsualBombNormalDuration()) * stepsTaken >= 0) // -280
			{
				...
				if (monIA.verbose)
					System.out
							.println(stepsTaken
									...
									+ " is "
									+ (m.representation[t.getLine()][t.getCol()] - (monIA
											.getCurrentToleranceCofficient() / monIA
											.getUsualBombNormalDuration())
											* stepsTaken));
				int cautionMargin = 0;
				if (monIA.isPredicting())
					cautionMargin = 10;
				if (m.representation[t.getLine()][t.getCol()]
						- (monIA.getCurrentToleranceCofficient() / monIA
								.getUsualBombNormalDuration()) * stepsTaken <= -300
						+ cautionMargin
						&& i != offset) {
					if (monIA.verbose)
					...
					if (monIA.verbose)
						System.out
								.println(m.representation[t.getLine()][t
										.getCol()]
										- stepsTaken
										* (monIA.getCurrentToleranceCofficient() / monIA
												.getUsualBombNormalDuration())
										...
										+ stepsTaken
										+ ","
										+ p.getTile(stepsTaken));
					if (safePlaceFoundBeforeDanger != -1) {
						if (monIA.verbose)
							System.out
									.println("But found an empty case before : "
											+ stepsTaken
											+ ","
											+ p.getTile(stepsTaken));
					} else if (!monIA.isInDangerAndWontMoveAnyway()
							|| (monIA.isPredicting() && p.getLastTile() == t)
							|| (monIA.isInDangerAndWontMoveAnyway() && m.representation[t
									.getLine()][t.getCol()] < m.representation[monIA
									.getSelfHero().getLine()][monIA
									.getSelfHero().getCol()]))
						return true;
				}
			}
			...
			if (monIA.getMode() == Mode.COLLECTE)
				if (stepsTaken > 0
						&& t.getHeroes().size() >= 2
						|| (t.getHeroes().size() == 1 && t.getHeroes().get(0) != monIA
								.getSelfHero())) {
					if (monIA.verbose)
			...
			if ((monIA.isPredicting() || !monIA.isInDangerAndWontMoveAnyway()
					&& monIA.getSelfHero().getTile().getBombs().size() > 0)
					&& (t.getHeroes().size() == 1 && t.getHeroes().get(0) != monIA
							.getSelfHero())) {
				if (monIA.verbose)
					System.out.println("POTENTIAL TRAP INCOMING");
				if (!monIA.isInDangerAndWontMoveAnyway())
					return true;
			}
	 ...
	 *             description manquante !
	 */
	public static boolean hasLastPathBecomeDangerousOrObsolete(Matrix m)
			throws StopRequestException {
		monIA.checkInterruption();
		if (monIA.verbose)
			System.out.println("Looking for obselete check");
