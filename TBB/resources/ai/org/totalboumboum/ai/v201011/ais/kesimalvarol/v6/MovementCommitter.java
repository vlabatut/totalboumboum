...
public class MovementCommitter {
	...
	/**
	 * Utilisee pour indiquer qu'on voudrait faire une operation distincte pour
	 * poser des bombes
	 */
	...
	/**
	 * On n'etait pas sur s'il y avait un bug dans A* et on n'avait pas trop du
	 * temps pour finaliser l'IA, donc, on avait developpée notre version de
	 * fonction successeur, qui est, en effet, une altération de
	 * BasicSuccessorCalculator.
	 * 
	 * Bien qu'on sache que chacune des noeuds representent un etat distincte et
	 * pas seulement une "case", on avait observee que la recherche nous prenait
	 * une temps absolument intolerable : Avant d'avoir implementé celle-ci, on
	 * pouvait observer un temps de calcul d'environ 15-20 secondes pour trouver
	 * un chemin vers une case complétement réguliere, notamment s'il existait
	 * des bombes ou des flammes. Maintenant, on ne voit aucune delai de calcul.
	 * 
	 * La difference est qu'on ne permette pas de chercher les cases auquels on
	 * avait arrivee déja par la case actuelle. On veut que A* nous renvoie
	 * seulement une chemin ; les significations des "états" nous empechent de
	 * faire un calcul vite. On sait que celle-ci peut détériorer l'éxecution
	 * des autres types de recherche, mais on avait vu une amélioration enorme
	 * pour notre agent.
	 * 
	 * par exemple, si la noeud courant symbolise un état de la case (10;10), on
	 * va chercher ses voisines qu'on peut transgresser. soit (9;10) l'une des
	 * telle voisines. Dans les iterations suivantes, si on arrive a (10;10) une
	 * deuxieme fois, on ne permettra pas de developper vers (9;10).
	 * 
	 */
	private class KesimalVarolSuccessor extends SuccessorCalculator {
		...
		public List<AiTile> processSuccessors(AstarNode node)
			...
			if (m.representation[node.getTile().getLine()][node.getTile()
					.getCol()] > -300
					&& node.getDepth() < (monIA.getZone().getHeight() + monIA
							.getZone().getWidth()) * 2) {
				if (discoveredByCases.get(node.getTile()) == null) {
					discoveredByCases.put(node.getTile(),
							new ArrayList<AiTile>());
				}
				ArrayList<AiTile> currTileList = discoveredByCases.get(node
						.getTile());
				AiTile tile = node.getTile();
				AiHero hero = node.getHero();

				// pour chaque case voisine : on la rajoute si elle est
				// traversable
				for (Direction direction : Direction.getPrimaryValues()) {
					monIA.checkInterruption();
					AiTile neighbor = tile.getNeighbor(direction);
					if (!currTileList.contains(neighbor)) {
						if ((node.getParent() == null || neighbor != node
								.getParent().getTile())
								&& neighbor.isCrossableBy(hero)) {
							result.add(neighbor);
	...
	/**
	 * Methode invoquee pour determiner l'action suivante, explicitement pour
	 * prevoir une bombe
	 * 
	 * @param m
	 *            La matrice sur laquelle nos calculs se baseront
	 * 
	 * @return L'action a utiliser
	 * @throws StopRequestException
	 *             description manquante !
	 */
	public AiAction commitMovementForFutureBomb(Matrix m)
			throws StopRequestException {
		monIA.checkInterruption();
	 ...
	 *            Oui si commitMovement est appelee pour prevoir si on peut
	 *            laisser une bombe ou non
	 * 
	 * @return L'action a utiliser
	 * @throws StopRequestException
	 *             description manquante !
	 */
	public AiAction commitMovement(Matrix m, boolean invokedForSafeBombPlacement)
			throws StopRequestException {
		monIA.checkInterruption();
		...
		// Si nous sommes dans le mode attaque, on veut savoir si on peut
		// trouver au moins une chemin vers une adversaire
		if (m.representation[monIA.getSelfHero().getLine()][monIA.getSelfHero()
				.getCol()] >= 0
				&& !monIA.isPredicting()
				&& monIA.getMode() == Mode.ATTAQUE) {
			boolean enablewalls = true;
			if (monIA.verbose)
				System.out.println("Bottleneck1");
			...
			if (m.representation[monIA.getSelfHero().getLine()][monIA
					.getSelfHero().getCol()] >= 0) {
				for (AiHero ah : monIA.getZone().getRemainingHeroes()) {
					monIA.checkInterruption();
					if (!ah.hasEnded() && ah != monIA.getSelfHero()) {
						AiTile tileToEvaluate = null;
						/**
						 * Ce n'est pas vraiment nécesssaire ; on l'avait
						 * utilisée avant notre version de fonction successeur,
						 * pour éviter les cases ayant des bombes car la
						 * recherche ne terminerait jamais. Mais notre
						 * utilisation du processeur n'est pas mal du tout et
						 * les resultats ne sont pas trop differentes du tout,
						 * donc on va le laisser tomber.
						 */
						for (AiTile atrgt : ah.getTile().getNeighbors()) {
							monIA.checkInterruption();
							if (atrgt.getBombs().size() == 0
									&& atrgt.getFires().size() == 0
									&& atrgt.getBlocks().size() == 0
									&& m.representation[atrgt.getLine()][atrgt
											.getCol()] >= 0
									&& (atrgt.getHeroes().size() == 0 || (atrgt
											.getHeroes().size() == 1 && atrgt
											.getHeroes().get(0) == monIA
											.getSelfHero())))
								tileToEvaluate = atrgt;
						}
						if (tileToEvaluate != null) {
							// A* : Si l'adversaire avait laissee une bombe au
							// moment de calcul, on aura une calcul immensement
							// lent.
							if (tileToEvaluate.getBombs().size() == 0
									&& tileToEvaluate.getFires().size() == 0) {
								AiPath path = null;
								try {
									sc.initNodeHack();
									path = astar.processShortestPath(monIA
											.getSelfHero().getTile(), ah
											.getTile());// tileToEvaluate);
								} catch (LimitReachedException e) {
									if (monIA.verbose)
										System.out.println("sdasdadad " + e);
								} catch (NullPointerException e) {
									if (monIA.verbose)
										System.out.println("sdasdadad2 " + e);
								}
								if (path != null && path.getLength() > 0) {
									enablewalls = false;
									if (monIA.getZone().getRemainingHeroes()
											.size() <= 2) // sinon, on veut le
															// tuer tout de
															// suite.
									{
										if (monIA.lastPathChosenParameters != null) {
											AiTile lastTile = monIA.lastPathChosenParameters
													.getLastPathChosen()
													.getLastTile();
											if (lastTile != ah.getTile()
													&& lastTile.getCol() != ah
															.getCol()
													&& lastTile.getLine() != ah
															.getLine())
			...
			if (monIA.verbose)
				System.out.println("Bottleneck2");
			// Si on n'a aucune chemin, alors on peut détruire les murs pour
			// créer une nous-meme.
			if (enablewalls) {
				// si on voudrait trouver une chemin probable vers le dernier
				// adversaire
				boolean willPlayNaturally = FastWallDestruction
						.canFindAndCreateBetterPathsIfThereIsOneEnemy(m, cc,
								hc, sc);

				if (willPlayNaturally) // sinon, on peut chercher
			...
			if (monIA.verbose)
				System.out.println("Bottleneck3");
		}

		if (!invokedForSafeBombPlacement) // si appel est pour emplacement des
											// bombes, on laisse celle-ci a
											// tomber
		{
			// Trouves-moi sur une case d'emplacement ?
			AiTile moniacurrtile = monIA.getSelfHero().getTile();
			if (moniacurrtile.getBombs().size() == 0) {
				if (m.regionEmplacementImportanceMatrix[moniacurrtile.getLine()][moniacurrtile
						.getCol()].importance > 0) {
					if (monIA.verbose)
						System.out.println("Is bomb permitted");
					if (monIA.isBombDropPermitted(m)) {
						monIA.willReleaseBomb();
						return new AiAction(AiActionName.NONE); // KesimalVarol
																// va faire la
																// posage ; on
																// veut qu'on la
																// pose dans la
			...
			if (bombSpecialRequest == null
					&& !PathSafetyDeterminators
							.hasLastPathBecomeDangerousOrObsolete(m)) {
				if (monIA.verbose)
					System.out.println("Nope, not obsolete");
				AiTile dest = monIA.lastPathChosenParameters
						.getLastPathChosen().getTile(
								monIA.lastPathChosenParameters
										.getCurrentPosition() + 1);
				/*
				 * if(dest==monIA.getSelfHero().getTile()) {
				 * monIA.updatePathIfChangedCase();
				 * dest=monIA.lastPathChosenParameters
				 * .getLastPathChosen().getTile
				 * (monIA.lastPathChosenParameters.getCurrentPosition()+1); }
				 */
				Direction dir = monIA.getZone().getDirection(
						monIA.getSelfHero().getTile(), dest);
		...
		if (result == null) {
			...
			if (!monIA.isPredicting()
					&& m.representation[monIA.getSelfHero().getLine()][monIA
							.getSelfHero().getCol()] < 0) {
				if (monIA.verbose)
					System.out.println("In danger, no way.");
	...
	/**
	 * La partie de commitMovement qui determine une chemin pour nous deplacer
	 * (et eventuellement, renvoie l'action suivante).
	 * 
	 * @param m
	 *            description manquante !
	 * @param invokedForSafeBombPlacement
	 *            description manquante !
	 * @return null si on ne peut pas decider sur une chemin, une action
	 *         concernant une deplacement vers la direction sinon.
	 * @throws StopRequestException
	 *             description manquante !
	 */
	private AiAction choosePath(Matrix m, boolean invokedForSafeBombPlacement)
			throws StopRequestException {
		monIA.checkInterruption();
		...
		if (monIA.verbose)
			System.out.println("Greatest variables found : ");
		for (int i = 0; i < 5; i++) {
			monIA.checkInterruption();
			if (monIA.verbose)
				System.out.println(hI[i] + "," + hJ[i] + ":" + hval[i]);
		}

		// a=4;
		// changedUpTo=4;
		if (monIA.verbose)
			System.out.println("Changedupto var : " + changedUpTo);
		...
		while (a < changedUpTo || !usedClosestCase) {
			...
			if (a == changedUpTo) {
				...
				if (monIA.verbose)
					System.out.println("Bomb special req : "
							+ bombSpecialRequest);
				if (bombSpecialRequest != null) {
					...
					if (b != null) {
						if (monIA.verbose)
							System.out.println("Bonus tile before bomb "
									+ target);
						target = b;
						a--;
						...
						currhval = m.representation[target.getLine()][target
								.getCol()];
					}
				}
			}
			...
			try {
				...
				if (monIA.getZone().getTileDistance(
						monIA.getSelfHero().getTile(), target) == 1) {
					pathToFollow = new AiPath();
					pathToFollow.addTile(monIA.getSelfHero().getTile());
					pathToFollow.addTile(target);
				} else if (target.getBombs().size() == 0
						&& target.getFires().size() == 0) {// Pas vraiment
															// necessaire mais
															// meme raison que
															// celui pour
				...
				if (retY == 2) {
					try {
						if (pathToFollow.getLength() >= 2) { // 2
							...
							if (monIA.verbose)
								System.out.println("Astar for target "
										+ pathToFollow.getLastTile());
							Direction dir = monIA.getZone().getDirection(
									monIA.getSelfHero().getTile(), dest);
							...
							if (monIA.verbose)
								System.out.println("Path too short for "
										+ target);
						}
					} catch (NullPointerException e) {
//						System.out.println("Case Nullptr " + e);
				...
				if (monIA.verbose)
					System.out.println("Astar limit " + e);
			} catch (NullPointerException e) {
				if (monIA.verbose)
					System.out.println("Astar Nullptr " + e);
			}
			a++;
		}
		return null;
