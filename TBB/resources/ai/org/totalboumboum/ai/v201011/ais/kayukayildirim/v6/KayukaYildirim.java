...
public class KayukaYildirim extends ArtificialIntelligence
{
	...
	private Direction actionDirection(AiTile tile1,AiTile tile2) throws StopRequestException {
		checkInterruption();
		Direction result;
		int dx;
		double dy;
	...
	{	
		...
		if(isModeCollecte())
			...
			try{
			if(matrice[ourOwnHero.getTile().getNeighbor(moveDir).getLine()][gameMap.getOwnHero().getTile().getNeighbor(moveDir).getCol()]>-250)
		...
		{	if(poserBombeAttack())
			...
			{
				getMouvementDirection(this.gameMap.getOwnHero().getTile(),getMaxTile());
				try{
					if(matrice[ourOwnHero.getTile().getNeighbor(moveDir).getLine()][gameMap.getOwnHero().getTile().getNeighbor(moveDir).getCol()]>-250)
						result = new AiAction(AiActionName.MOVE,this.moveDir);
					else
...
private void fillCollecte(double[][] matriceCollecte,AiZone gameMap)throws StopRequestException{
	checkInterruption();
	...
	private void fillAttaque(double[][] matriceCollecte,AiZone gameMap)throws StopRequestException{
		checkInterruption();
		this.initialiseMap(matriceCollecte);
		this.fillwithInDes(matriceCollecte);
		this.fillwithBombs(matriceCollecte);
	...
	private void initialiseMap(double[][] matriceCollect) throws StopRequestException {
		checkInterruption();
	...
	private void fillwithInDes(double[][] matriceCollect) throws StopRequestException{
		checkInterruption();
	...
	private void fillwithDes(double[][] matriceCollect) throws StopRequestException{
		checkInterruption();
		Collection<AiBlock> allblocks = gameMap.getBlocks();
		Iterator<AiBlock> itallblocks = allblocks.iterator();
		while(itallblocks.hasNext()){
			...
			matriceCollect[(block.getLine()-1)%gameMap.getWidth()][block.getCol()]+=10;
			matriceCollect[(block.getLine()+1)%gameMap.getWidth()][block.getCol()]+=10;		
			matriceCollect[block.getLine()][(block.getCol()-1)%gameMap.getHeight()]+=10;
			matriceCollect[block.getLine()][(block.getCol()+1)%gameMap.getHeight()]+=10;
	...
	private void fillwithBombs(double[][] matriceCollect) throws StopRequestException{
		checkInterruption();
		...
		while (iteratorBombs.hasNext()) 
		{
			...
			while (iteratorScope.hasNext())
			{
				...
				matrice[blastCase.getLine()][blastCase.getCol()] +=(int) ((-100*(bomb.getTime()))/1000);
				
			}
		}
	}
	private int distancePlusProcheAdversaire(int l, int c) throws StopRequestException{
		checkInterruption();
		...
		while(itheroes.hasNext())
		{
			...
			{   d = (int)gameMap.getPixelDistance(l, c, hero.getLine(), hero.getCol());
				if(d<min)
					min=d;
			}
		 }
	...
	private void fillwithBns(double[][] matriceCollect) throws StopRequestException{
		checkInterruption();
		...
		while(itbns.hasNext()){
			...
			if(bns.getType().toString()=="EXTRA_BOMB"){
				if(gameMap.getPixelDistance(x, y, bns.getLine(), bns.getCol())==0)
					matriceCollect[bns.getLine()][bns.getCol()] +=(int)(BOMBBNS*(3/ourOwnHero.getBombNumberMax())+Z-Z/distancePlusProcheAdversaire(bns.getLine(),bns.getCol()));
				else
				matriceCollect[bns.getLine()][bns.getCol()] +=(int)(BOMBBNS*(3/ourOwnHero.getBombNumberMax())+Z/gameMap.getPixelDistance(x, y, bns.getLine(), bns.getCol())-Z/distancePlusProcheAdversaire(bns.getLine(),bns.getCol()));
			}
			else{
				if(gameMap.getPixelDistance(x, y, bns.getLine(), bns.getCol())==0)
					matriceCollect[bns.getLine()][bns.getCol()] +=(int)(EFBNS*Z/(4*ourOwnHero.getBombRange())+Z-Z/distancePlusProcheAdversaire(bns.getLine(),bns.getCol()));	
				else
				matriceCollect[bns.getLine()][bns.getCol()] +=(int)(EFBNS*Z/(4*ourOwnHero.getBombRange())+Z/gameMap.getPixelDistance(x, y, bns.getLine(), bns.getCol())-Z/distancePlusProcheAdversaire(bns.getLine(),bns.getCol()));	
			}
		 }
	}
	private void takeHeroes(double [][] matriceCollect) throws StopRequestException{
		checkInterruption();
		Collection<AiHero> allheroes = gameMap.getHeroes();
		Iterator<AiHero> itheroes = allheroes.iterator();
		while(itheroes.hasNext()){
	...
	private void takeFireHoles(double [][] matriceCollect) throws StopRequestException{
		checkInterruption();
	...
	private void printMatrixtoConsole(double[][] matriceCollect) throws StopRequestException
	{checkInterruption();
		int i=0;
	...
	private void printOnScreen(double[][] matrice,AiOutput out) throws StopRequestException
	{  checkInterruption();
	...
	private void aFillWithBns(double[][] matriceAttaque) throws StopRequestException{
		checkInterruption();
		int x = ourOwnHero.getLine();
		int y = ourOwnHero.getCol();
		int Z=gameMap.getHeight()+gameMap.getWidth();
		...
		while(itbns.hasNext()){
			...
			if(bns.getType().toString()=="EXTRA_BOMB"){
				if(gameMap.getPixelDistance(x, y, bns.getLine(), bns.getCol())==0)
					matriceAttaque[bns.getLine()][bns.getCol()] +=(int)(10*(3/ourOwnHero.getBombNumberMax())+Z-Z/distancePlusProcheAdversaire(bns.getLine(),bns.getCol()));
				else
				matriceAttaque[bns.getLine()][bns.getCol()] +=(int)(10*(3/ourOwnHero.getBombNumberMax())+Z/gameMap.getPixelDistance(x, y, bns.getLine(), bns.getCol())-Z/distancePlusProcheAdversaire(bns.getLine(),bns.getCol()));
			}
			else{
				if(gameMap.getPixelDistance(x, y, bns.getLine(), bns.getCol())==0)
					matriceAttaque[bns.getLine()][bns.getCol()] +=(int)(10*Z/(4*ourOwnHero.getBombRange())+Z-Z/distancePlusProcheAdversaire(bns.getLine(),bns.getCol()));	
				else
				matriceAttaque[bns.getLine()][bns.getCol()] +=(int)(10*Z/(4*ourOwnHero.getBombRange())+Z/gameMap.getPixelDistance(x, y, bns.getLine(), bns.getCol())-Z/distancePlusProcheAdversaire(bns.getLine(),bns.getCol()));	
			}
		 }
	}
	...
	private void aTakeHeroes(double [][] matriceAttaque) throws StopRequestException{
			checkInterruption();
			int Z=gameMap.getHeight()+gameMap.getWidth();
			int x,y,blast,i;
			Collection<AiHero> allheroes = gameMap.getHeroes();
				...
				if(hero!=gameMap.getOwnHero())
				{
				    ...
				    for(i=1; i< blast+1 ;i++)
				    {
				    	...
				    	if(	matriceAttaque[x][y+i]!=-400){
				    		if(gameMap.getPixelDistance(ourOwnHero.getLine(), ourOwnHero.getCol(), hero.getLine(), hero.getCol())<1)
				    			matriceAttaque[x][superMod(y+i,gameMap.getWidth())]+=(int) (BADV*(Z/1));
				    		else
				    			matriceAttaque[x][superMod(y+i,gameMap.getWidth())]+=(int) (BADV*(Z/gameMap.getPixelDistance(ourOwnHero.getLine(), ourOwnHero.getCol(), hero.getLine(), hero.getCol())));				    	
					    		 if( matriceAttaque[x][superMod(y+i,gameMap.getWidth())]<200)
					    			 matriceAttaque[x][superMod(y+i,gameMap.getWidth())]+=200; 
					    	}    
				    	else
				    		i=blast;
				    }
				    ...
				    for(i=1; i< blast+1 ;i++)
				    {
				    	...
				    	if(	matriceAttaque[x+i][y]!=-400){
				    		if(gameMap.getPixelDistance(ourOwnHero.getLine(), ourOwnHero.getCol(), hero.getLine(), hero.getCol())<1)
				    			matriceAttaque[superMod((x+i),gameMap.getHeight())][y]+=(int) (BADV*(Z/1));
				    		else
				    		matriceAttaque[superMod((x+i),gameMap.getHeight())][y]+=(int) (BADV*(Z/gameMap.getPixelDistance(ourOwnHero.getLine(), ourOwnHero.getCol(), hero.getLine(), hero.getCol())));				    
				    		if(matriceAttaque[superMod((x+i),gameMap.getHeight())][y]>200)
				    			matriceAttaque[superMod((x+i),gameMap.getHeight())][y]+=200;}
				    	else
				    		i=blast;
				    }
				    for(i=1; i< blast+1 ;i++)
	...
	private boolean poserBombeCollecte() throws StopRequestException{
		...
		if(runAway(ourOwnHero.getTile())==true)
			if((!accessible(ourOwnHero)&& listOfBlocksInBlast(ourOwnHero.getTile(),ourOwnHero.getBombRange()).size()>0))
		...
		{	
			if(isRunnable(ourOwnHero.getTile())==true){
				//if(gameMap.getHiddenItemsCount()>0)
					if(getWillBurnWalls()>0)
						dropBomb=true;
					else
						dropBomb=false;
				//else
	...
	private boolean poserBombeAttack() throws StopRequestException{
		...
		boolean dropBomb=false;
		if(runAway(ourOwnHero.getTile())==true)
			if(heroesInRange(ourOwnHero.getTile(),ourOwnHero.getBombRange()).size()>0 || (!accessible(ourOwnHero)&& listOfBlocksInBlast(ourOwnHero.getTile(),ourOwnHero.getBombRange()).size()>0))
			dropBomb=true;
		return dropBomb;
			
	}

	private List<AiHero> heroesInRange(AiTile tile, int range) throws StopRequestException{
		checkInterruption();
	...
	private boolean accessible(AiHero hero) throws StopRequestException{
		...
		for(Direction dir:dirTable){
			checkInterruption();
			if(tile.getNeighbor(dir).isCrossableBy(ourOwnHero, false, false, false, true, true, true))
					i++;}
		if(i>2)
			bool=true;
		return bool;
	...
	public List<AiTile> getBombRangeList(AiTile tile, int range) throws StopRequestException{
		...
		for(Direction dir:this.dirTable){
			...
			while(i < range && crossable==true){
				...
				if(neighbourTile.isCrossableBy(gameMap.getOwnHero()) && crossableItem){
					i++;
					tile = neighbourTile;
	...
	public boolean runAway(AiTile bomb) throws StopRequestException{
		...
		for(Direction dir:dirTable){
			checkInterruption();
			allRangeList.addAll(getONeighbour(bomb, dir, gameMap.getOwnHero().getBombRange()));
		}
	...
	private void getMouvementDirection(AiTile tile1, AiTile tile2) throws StopRequestException, NullPointerException{	
		checkInterruption();
		...
		if(this.path.getTiles().size()!=0){
			...
			this.moveDir = this.gameMap.getDirection(tile1, tempPath2.getLastTile());
		}
		else{
...
private List<AiBlock> listOfBlocksInBlast(AiTile tile, int range) throws StopRequestException{
	checkInterruption();
