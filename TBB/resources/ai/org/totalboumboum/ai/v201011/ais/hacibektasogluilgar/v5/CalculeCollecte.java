...
private AiAction decisionBombPourCollecte(Etats[][] matriceZone,double matriceTotal[][],AiPath chemin) throws StopRequestException, LimitReachedException
{
	...
	if(matriceTotal[heroLine][heroColonne]>0)
	{
		...
		{
			if(matriceZone[heroLine][heroColonne]==Etats.VIDE_AU_MUR_DESTRUCTIBLE 
					|| matriceZone[heroLine][heroColonne]==Etats.ADVERSAIRE 
					|| matriceZone[heroLine][heroColonne]==Etats.VIDE_PRIORITAIRE)
			{
				...
				if(control)	
				{
					...
					if(chemin.getLength()>0)
					{
						...
						matriceTotal[notreHeros.getLine()][notreHeros.getCol()]=0;
				}
	...
	for(int i=0;i< cz.hauteur;i++)
	{
		...
		for(int j=0;j< cz.largeur;j++)
		{
			...
			case MUR_DESTRUCTIBLE:
				 ...
				 act.calculeDapresPortee( matriceTotal,notreHeros,i, j, 10,1);
				break;
			...
			case ADVERSAIRE:
				...
				if(distance==0) distance=1;
				 ...
				 act.calculeDapresPortee( matriceTotal,notreHeros,i, j, 20,2);
				break;					
			case BONUS:
				/*
