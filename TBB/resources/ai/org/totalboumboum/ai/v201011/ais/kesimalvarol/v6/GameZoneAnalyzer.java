...
/**
 * La classe qui conduisse les op√©rations concernant la zone du jeu et les
 * matrices.
 * 
...
public class GameZoneAnalyzer {
	 ...
	 * 
	 * @param gOld
	 *            Une objet GameZone -probablement celui de l'iteration
	 *            precedente- pour initialisation . Peut etre null si c'est la
	 *            premiere fois qu'on l'utilise (valeurs statiques ne sont pas
	 *            utilisees comme on aura obtenu des resultats fausses aux
	 *            manches suivantes)
	 * @throws StopRequestException
	 *             description manquante !
	 */
	...
	public Matrix constructInterestMatrix() throws StopRequestException {
		 ...
		 * Dans le 1er, on va initialiser l'arbre au dessus. On mettra les
		 * valeurs dans l'arbre, ayant des clés étant les valeurs d'interet.
		 * Dans la 2e, grace au nature des clés, on va les separer aux couches
		 * (en fonction des les valeurs d'interet). Chacune des bombes dans un
		 * couche va "attirer" les bombes dans les couches superieures qu'ils
		 * peuvent declencher. Celle-ci signifie que les bombes aux couches
		 * superieures peuvent etre detruites plus tot. Finalement, on va
		 * ajouter les valeurs dans la matrice d'interet.
		 */
		// Partie 1
		for (AiBomb bomb : cbombs) {
			...
			double lowestNeighboringValue = m.representation[bombpos.getLine()][bombpos
					.getCol()];
			for (AiTile voisines : bombpos.getNeighbors()) {
				monIA.checkInterruption();
				double tmp = m.representation[voisines.getLine()][voisines
						.getCol()];
				AiTile voisinDevoisin = voisines.getNeighbor(monIA.getZone()
						.getDirection(bombpos, voisines));
				if (voisinDevoisin != null)
					if (m.representation[voisinDevoisin.getLine()][voisinDevoisin
							.getCol()] == tmp) {
			...
			double bombPassTolerance = lowestNeighboringValue
					- (monIA.getCurrentToleranceCofficient() / monIA
			...
			if (bombPassTolerance <= -300)
				...
				delay = -300 + 100.0
						* (delay / (double) bomb.getNormalDuration());

			ArrayList<bombDetailed> tmp = bombsCurrentMapForOrdering.get(Double
					.valueOf(delay));
			if (tmp == null) {
		...
		for (Double bKey : bombsCurrentMapForOrdering.keySet()) {
			...
			while (bCurrentIter.hasNext()) {
				...
				for (Double bKeyLower : bombsCurrentMapForOrdering.keySet()) {
					monIA.checkInterruption();
					if (bKeyLower > bKey) {
						ArrayList<bombDetailed> tmpLower = bombsCurrentMapForOrdering
								.get(bKeyLower);
						Iterator<bombDetailed> potentialAffectedIter = tmpLower
								.iterator();
						while (potentialAffectedIter.hasNext()) {
							monIA.checkInterruption();
							bombDetailed potentialAffected = (bombDetailed) potentialAffectedIter
									.next();
							for (AiTile t : affectedCases) {
								monIA.checkInterruption();
								if (t != bCurrent.bomb.getTile()) {
									if (t.getBombs().contains(
											potentialAffected.bomb)) {
										potentialAffected.delay = bKey;
										tmp.add(potentialAffected);
										tmpLower.remove(potentialAffected);
										potentialAffectedIter = tmpLower
												.iterator();
	 ...
	 *             description manquante !
	 */
	private void assignBasicNums(Matrix m, int i, int j)
			throws StopRequestException {
		monIA.checkInterruption();
		AiTile currtile = monIA.getZone().getTile(i, j);
		...
		if (monIA.getMode() == Mode.COLLECTE)
			modifier = 50;
		else if (monIA.getZone().getHeroes().size()
				- monIA.getZone().getRemainingHeroes().size() > 2) {
			modifier = 10;
		} else
			// Si on a seulement une adversaire, elle ne sera pas tuee par les
			// autres, on ignore les bonus completement
			modifier = 0;
		...
		if (m.representation[i][j] >= 0
				&& currtile.isCrossableBy(monIA.getSelfHero())) {
			int awardedval = 2 * ((azone.getHeight() + azone.getWidth()) - azone
					.getTileDistance(ourtile, currtile)) / 3;
	...
	private void handleWallAndEnemies(Matrix m) throws StopRequestException {
		...
		for (AiHero adv : adversaires) {
			...
			if (!adv.hasEnded() && adv != monIA.getSelfHero()) {
				...
				tAll.add(ctile.getNeighbor(Direction.DOWN).getNeighbor(
						Direction.RIGHT));
				tAll.add(ctile.getNeighbor(Direction.DOWN).getNeighbor(
						Direction.LEFT));
				tAll.add(ctile.getNeighbor(Direction.UP).getNeighbor(
						Direction.RIGHT));
				tAll.add(ctile.getNeighbor(Direction.UP).getNeighbor(
						Direction.LEFT));
				for (AiTile t : tAll) {
				...
				m.regionEmplacementImportanceMatrix[ctile.getLine()][ctile
						.getCol()].importance += 1;
				m.regionEmplacementImportanceMatrix[ctile.getLine()][ctile
						.getCol()].forEnemy = true;
	...
	/**
	 * Les murs peuvent etre geree separement d'adversaires (mode attaque),
	 * celle-ci nous permet de le faire
	 * 
	 * @param m
	 *            Matrice pour determiner les valeurs
	 * @param modifier
	 *            La valeur a utiliser lors de affectation des valeurs.
	 * @throws StopRequestException
	 *             description manquante !
	 */
	public void handleWalls(Matrix m, int modifier) throws StopRequestException {
		...
		for (AiBlock ablock : monIA.getZone().getDestructibleBlocks()) {
			...
			for (AiTile t : ablock.getTile().getNeighbors()) {
				monIA.checkInterruption();
				if (t.getBlocks().size() == 0
						&& m.representation[t.getLine()][t.getCol()] >= 0) {
					if (true) {
						m.markForBombPlacementCandidate(ablock, t.getLine(),
								t.getCol(), modifier);
					}
				}
			}
