...
public class CalculeAttaque {
	 ...
	 *             description manquante !
	 */
	public CalculeAttaque(HacibektasogluIlgar hi, CalculeZone cz, Action act)
			throws StopRequestException {
	...
	/**
	 * pour le mode attaque on calcule s'il est necessaire de poser une bomb ou
	 * pas
	 * 
	 * @param matriceZone
	 * 		description manquante !
	 ...
	 * 		description manquante !
	 */
	private AiAction decisionBombPourAttaque(Etats[][] matriceZone,
	 ...
	 * 		description manquante !
	 */
	public AiAction calculeMatriceAttaque(Etats matrice[][],
			...
			for (int j = 0; j < cz.largeur; j++) {
				...
				case MUR_DESTRUCTIBLE:
					...
					for (int q = 0; q < cz.nbAdversaire; q++) {
						hi.checkInterruption();
						int distance = Math.abs(cz.adversairesActuelles[q]
								.getLine() - i)
								+ Math.abs(cz.adversairesActuelles[q].getCol()
										- j);
						if (distance == 0 && matriceTotal[i][j] >= 0)
							note += 400;
						else
							note += 400 / distance;
					}
				...
				// on donne le couleur rouge aux tiles inaccesible par notre
				// héro
