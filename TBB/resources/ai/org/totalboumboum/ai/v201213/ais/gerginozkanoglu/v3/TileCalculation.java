...
public class TileCalculation {
	...
	protected boolean blockingEnemyIsPossible(AiTile ourTile)
		...
		if (enemyControl) {
			...
			for (int i = 0; i < bombRange; i++) {
				ai.checkInterruption();
				neighbor = neighbor.getNeighbor(Direction.UP);
				if (neighbor.getNeighbor(Direction.LEFT).isCrossableBy(ourHero)
				...
				if (i == bombRange - 1
						&& neighbor.getNeighbor(Direction.UP).isCrossableBy(
								ourHero)) {
					directionControl = false;
					break;
				}
		...
		if (enemyControl) {
			...
			for (int i = 0; i < bombRange; i++) {
				ai.checkInterruption();
				neighbor = neighbor.getNeighbor(Direction.LEFT);
				if (neighbor.getNeighbor(Direction.UP).isCrossableBy(ourHero)
						|| neighbor.getNeighbor(Direction.DOWN).isCrossableBy(
								ourHero)) {
					directionControl = false;
					break;
				...
				if (i == bombRange - 1
						&& neighbor.getNeighbor(Direction.LEFT).isCrossableBy(
								ourHero)) {
					directionControl = false;
					break;
				}
	...
	public AiHero closestEnemyToUs(AiTile ourTile) throws StopRequestException 
	{
		...
		for(AiHero enemy : this.ai.getZone().getRemainingHeroes())
		{
			ai.checkInterruption();
