...
/**
 * Cette classe est un simple exemple de critère binaire, pour comprendre force
 * de l'adversaire.
 * 
 * @author Banu Erdem
 * @author Zübeyir Tayyar
 */
public class ForceEnnemie extends AiUtilityCriterionInteger<ErdemTayyar> {
	/** */
	...
	public Integer processValue(AiTile tile) throws StopRequestException {
		...
		while (i < 5 && !result1)// Il regarde tous les tiles voisins du tile
									// pour trouver l'adversaire
		{
			ai.checkInterruption();
			int j = 1;
			while (j < range + 1 && !result1) {
				ai.checkInterruption();
				if (tile.getRow() + k * j < zone.getHeight()
						&& tile.getCol() + l * j < zone.getWidth()
						&& tile.getRow() + k * j >= 0
						&& tile.getCol() + l * j >= 0) {

					tile_voisin = zone.getTile(tile.getRow() + k * j,
							tile.getCol() + l * j);
		...
		// Si le tile est proche de l'adversaire plus faible dans la zone , il
		// retourne true
		if (tile_hero != null) {
			if (tile_hero.getHeroes().get(0).getRoundRank() <= myhero
					.getRoundRank() && !tile_hero.getHeroes().contains(myhero)) {
				// System.out.println("plus faible que notre hero");
				result = 2;
			} else if (!tile_hero.getHeroes().contains(myhero))
				result = 1;
