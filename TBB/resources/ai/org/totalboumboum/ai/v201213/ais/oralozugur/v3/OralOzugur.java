	...
	public AiHero getNearestEnemy() throws StopRequestException {
		...
		if (enemy_accessible) {
			for (AiHero enemy : zone.getRemainingOpponents()) {
				this.checkInterruption();
				if ((simpleTileDistance(hero.getTile(), enemy.getTile()) < distance)
		...
		else
		{
			for (AiHero enemy : zone.getRemainingOpponents()) {
				this.checkInterruption();
				if ((simpleTileDistance(hero.getTile(), enemy.getTile()) < distance) && (enemy.getId() != getZone().getOwnHero().getId())){
