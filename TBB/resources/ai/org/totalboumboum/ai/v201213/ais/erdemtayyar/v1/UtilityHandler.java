...
import org.totalboumboum.ai.v201213.ais.erdemtayyar.v1.criterion.Concurrence;
import org.totalboumboum.ai.v201213.ais.erdemtayyar.v1.criterion.ForceEnnemie;
import org.totalboumboum.ai.v201213.ais.erdemtayyar.v1.criterion.MurDestructible;
import org.totalboumboum.ai.v201213.ais.erdemtayyar.v1.criterion.MurDestructibleMenace;
import org.totalboumboum.ai.v201213.ais.erdemtayyar.v1.criterion.Pertinence;
import org.totalboumboum.ai.v201213.ais.erdemtayyar.v1.criterion.Securite;
import org.totalboumboum.ai.v201213.ais.erdemtayyar.v1.criterion.Temps;
...
public class UtilityHandler extends AiUtilityHandler<ErdemTayyar> {
	...
	protected void initCases() throws StopRequestException {
		...
		criteria.add(criterionMap.get(MurDestructibleMenace.NAME));
		new AiUtilityCase(ai, CASE_NAME2, criteria);

		// on définit un troiseme cas utilisant les trois  critères
		criteria = new TreeSet<AiUtilityCriterion<?, ?>>();
		criteria.add(criterionMap.get(Securite.NAME));
		criteria.add(criterionMap.get(ForceEnnemie.NAME));
		criteria.add(criterionMap.get(Temps.NAME));
	...
	protected void initReferenceUtilities() throws StopRequestException {
		...
		// on commence avec le mode collecte
		{
			...
			}
			{
				...
				combi.setCriterionValue((MurDestructibleMenace) criterionMap
						.get(MurDestructibleMenace.NAME), Boolean.FALSE);
				defineUtilityValue(mode, combi, utility);
				utility++;
			}
			{
			...
			}
			{
				...
				combi.setCriterionValue((MurDestructible) criterionMap
						.get(MurDestructible.NAME), 0);
				combi.setCriterionValue((MurDestructibleMenace) criterionMap
						.get(MurDestructibleMenace.NAME), Boolean.TRUE);
				defineUtilityValue(mode, combi, utility);
				utility++;
			}
			{
		...
		// on traite maintenant le mode attaque
		{
			...
			
			{
				...
				combi.setCriterionValue(
						(Securite) criterionMap.get(Securite.NAME), 0);
				combi.setCriterionValue(
						(ForceEnnemie) criterionMap.get(ForceEnnemie.NAME),
						Boolean.FALSE);
				combi.setCriterionValue((Temps) criterionMap.get(Temps.NAME),
						Boolean.FALSE);
				defineUtilityValue(mode, combi, utility);
			...
			}
			{
				...
				combi.setCriterionValue(
						(Temps) criterionMap.get(Temps.NAME), Boolean.FALSE);
				combi.setCriterionValue((MurDestructible) criterionMap
						.get(MurDestructible.NAME), 0);
				defineUtilityValue(mode, combi, utility);
				utility++;
			...
			}
			{
				combi = new AiUtilityCombination(caseMap.get(CASE_NAME4));
				combi.setCriterionValue(
						(Temps) criterionMap.get(Temps.NAME), Boolean.TRUE);
				combi.setCriterionValue((MurDestructible) criterionMap
						.get(MurDestructible.NAME), 0);
				defineUtilityValue(mode, combi, utility);
				utility++;
