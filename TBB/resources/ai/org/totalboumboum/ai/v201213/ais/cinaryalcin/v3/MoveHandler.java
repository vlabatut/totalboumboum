...
{	
	...
	ArrayList<AiTile> firelist=new ArrayList<AiTile>();
	for(AiFire fire:zone.getFires())
	{
		ai.checkInterruption();	
		if(!firelist.contains(fire.getTile()))
			firelist.add(fire.getTile());
	}
	...
	if(!herocontrol&& !ai.getCurrentDangerousTiles().contains(currentDestination))
	{
			...
			while(it.hasNext())
			{
				...
				if(currentDestination!=null && currentDestination.equals(ownHero.getTile())&&utility>0)
				{
