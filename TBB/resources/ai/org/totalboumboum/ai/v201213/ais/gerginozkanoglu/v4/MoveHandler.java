...
{	
	...
	{	
	   ...
	   if(utilities.containsKey(currentDestination))
	   {
		   if(ai.getZone().getOwnHero().getTile().equals(currentDestination))
	    	...
	    	else if( 1.2*utilities.get(currentDestination) >= utilities.get(biggestTile))
	    	{
	...
	{	
		...
		AiPath path = pathOperation.bestPath(this.ai.getZone().getOwnHero(), currentDestination);
		return path;
	}
	...
	else
	{
		...
		try
		{
			...
			if(path.getLocations().size() == 1 || path == null)
				path = pathCalculation.bestPath(this.ai.getZone().getOwnHero(), calculate.closestAndSafestTileForHero(this.ai.getZone().getOwnHero()));
			if(!pathCalculation.isNextTileOnPathSafe(path.getLocation(NEXT_LOCATION).getTile()))
			{
