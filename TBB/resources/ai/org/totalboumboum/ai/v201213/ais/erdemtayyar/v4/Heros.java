...
import org.totalboumboum.ai.v201213.ais.erdemtayyar.v4.Tiles;
...
public class Heros {
	 ...
	 */
	public Heros(ErdemTayyar ai) throws StopRequestException {
...
// METHODS
	...
	protected boolean isHeroInDanger() throws StopRequestException {
		ai.checkInterruption();
		Tiles to = new Tiles(this.ai);
		to.setCurrentDangerousTiles();
		if (to.getCurrentDangerousTiles().contains(this.ai.getHero().getTile()))
			return true;
		return false;
	}
	...
	protected boolean heroInEndlessLoop() throws StopRequestException {
		...
		if (this.ai.getLastUtilityTiles().size() > ENDLESS_LOOP_LIMIT) {
			HashSet<AiTile> set = new HashSet<AiTile>(
					this.ai.getLastUtilityTiles());
	 ...
	 *             If the engine demands the termination of the agent.
	 */
	protected boolean isHeroInRange(AiHero givenHero)
			throws StopRequestException {
		ai.checkInterruption();
		...
		for (AiHero currentHero : this.ai.getZone().getRemainingHeroes()) {
			...
			if (!currentHero.equals(givenHero)) {
				Tiles ts = new Tiles(this.ai);
				if (ts.getDangerousTilesOnBombPut(givenHero.getTile(),
						givenHero.getBombRange()).contains(
						currentHero.getTile()))
					return true;
			}
		}
	...
	protected boolean canReachSafety(AiHero givenHero)
		...
		Tiles to = new Tiles(this.ai);
		to.setCurrentDangerousTiles();
		ArrayList<AiTile> accessibleTiles = to.getAccessibleTiles(givenHero
				.getTile());
		int safeTileCount = accessibleTiles.size();
		for (AiTile currentTile : accessibleTiles) {
			ai.checkInterruption();
			if (to.getDangerousTilesOnBombPut(givenHero.getTile(),
					givenHero.getBombRange()).contains(currentTile)
					|| to.getCurrentDangerousTiles().contains(currentTile))
				safeTileCount--;
		}
