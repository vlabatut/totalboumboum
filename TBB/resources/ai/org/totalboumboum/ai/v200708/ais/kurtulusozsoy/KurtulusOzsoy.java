...
public class KurtulusOzsoy extends ArtificialIntelligence {
	...
	public Integer processAction() throws Exception {
		...
		if (firstTime)
			...
			if (dangerPos[0] != -1) { // on calcule l'ordre des directions
				int temp[] = getDirectionPreferences(x, y, dangerPos[0],
						dangerPos[1]);
				// on sélectionne une direction possible
				int i = 0;
				while (i < temp.length
				...
				if (getTimeBeforeShrink() < 7000) {
					...
					result = getBonus(
							x,
							y,
							getClosestBonusPosition(x, y, AI_BLOCK_ITEM_FIRE)[0],
							getClosestBonusPosition(x, y, AI_BLOCK_ITEM_FIRE)[1]);
					if (result == AI_ACTION_DO_NOTHING) {
						result = getBonus(
								x,
								y,
								getClosestBonusPosition(x, y,
										AI_BLOCK_ITEM_BOMB)[0],
								getClosestBonusPosition(x, y,
										AI_BLOCK_ITEM_BOMB)[1]);
					}
					if (result == AI_ACTION_DO_NOTHING) {
						result = move(x, y);
					}
	...
	{
		if ((Math.abs(x - dangerX) == 0 && (Math.abs(y - dangerY) <= getBombPowerAt(
				dangerX, dangerY)))
				|| (Math.abs(y - dangerY) == 0 && (Math.abs(x - dangerX) <= getBombPowerAt(
						dangerX, dangerY)))) {
			...
			if (Math.abs(y - dangerY) != 0) {
				...
				if (isMovePossible(x, y, AI_ACTION_GO_UP)
						&& (isMovePossible(x, y - 1, AI_ACTION_GO_UP)
								|| isMovePossible(x, y - 1, AI_ACTION_GO_LEFT) || isMovePossible(
								x, y - 1, AI_ACTION_GO_RIGHT))) {
					indexUp = 0;
					indexDn = 1;
					indexLt = 2;
					indexRt = 3;
				} else if (isMovePossible(x, y, AI_ACTION_GO_DOWN)
						&& (isMovePossible(x, y + 1, AI_ACTION_GO_DOWN)
								|| isMovePossible(x, y + 1, AI_ACTION_GO_LEFT) || isMovePossible(
								x, y + 1, AI_ACTION_GO_RIGHT))) {
					indexDn = 0;
					indexUp = 1;
					...
					indexRt = 3;
						...
						&& (isMovePossible(x + 1, y, AI_ACTION_GO_DOWN)
								|| isMovePossible(x + 1, y, AI_ACTION_GO_UP) || isMovePossible(
								x + 1, y, AI_ACTION_GO_RIGHT))) {
					indexRt = 0;
					indexDn = 2;
					indexUp = 3;
					indexLt = 1;
	...
	private int move(int x, int y) {
		...
		} else if (possibleMoves.size() > 0) { // on peut poser une bombe si on
												// est à la fois dans un cul de
												// sac
												// (1 seul déplacement possible)
