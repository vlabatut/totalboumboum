...
public class KilicPala extends ArtificialIntelligence {
	...
	public synchronized Integer processAction() throws Exception {
		...
		if (firstTime)
			...
			if ((getTimeBeforeShrink() <= 1000) && (distShrink < 5)) {
				...
				if ((getNextShrinkPosition()[0] <= getZoneMatrixDimX() / 2)
						&& (getNextShrinkPosition()[1] <= getZoneMatrixDimY() / 2)) {
					try {
						if (isNextPosMoreFar(1, 1, x, y, possibleMoves.get(0))
								&& possibleMoves.get(0) == whichMoveMakesMeLeastFar(
										xShrink, yShrink, x, y))
							result = possibleMoves.get(0);
						else if (isNextPosMoreFar(1, 1, x, y,
								possibleMoves.get(1))
				...
				else if ((getNextShrinkPosition()[0] >= getZoneMatrixDimX() / 2)
						&& (getNextShrinkPosition()[1] <= getZoneMatrixDimY() / 2)) {
					try {
						if (isNextPosMoreFar(getZoneMatrixDimX(), 1, x, y,
								possibleMoves.get(0))
								&& possibleMoves.get(0) == whichMoveMakesMeLeastFar(
										xShrink, yShrink, x, y))
							result = possibleMoves.get(0);
						else if (isNextPosMoreFar(getZoneMatrixDimX(), 1, x, y,
								possibleMoves.get(1))
								&& possibleMoves.get(1) == whichMoveMakesMeLeastFar(
										xShrink, yShrink, x, y))
							result = possibleMoves.get(1);
				...
				// si shrink est en bas et a droite
						...
						&& (getNextShrinkPosition()[1] >= getZoneMatrixDimY() / 2)) {
					try {
						if (isNextPosMoreFar(getZoneMatrixDimX(),
								getZoneMatrixDimY(), x, y, possibleMoves.get(0))
								&& possibleMoves.get(0) == whichMoveMakesMeLeastFar(
										xShrink, yShrink, x, y))
							result = possibleMoves.get(0);
						else if (isNextPosMoreFar(getZoneMatrixDimX(),
								getZoneMatrixDimY(), x, y, possibleMoves.get(1))
								&& possibleMoves.get(1) == whichMoveMakesMeLeastFar(
										xShrink, yShrink, x, y))
							result = possibleMoves.get(1);
						else if (isNextPosMoreFar(getZoneMatrixDimX(),
								getZoneMatrixDimY(), x, y, possibleMoves.get(2))
								&& possibleMoves.get(2) == whichMoveMakesMeLeastFar(
										xShrink, yShrink, x, y))
							result = possibleMoves.get(2);
						else if (isNextPosMoreFar(getZoneMatrixDimX(),
								getZoneMatrixDimY(), x, y, possibleMoves.get(3))
								&& possibleMoves.get(3) == whichMoveMakesMeLeastFar(
										xShrink, yShrink, x, y))
							result = possibleMoves.get(3);
						else {
				...
				// si shrink est en bas et a gauche
						...
						&& (getNextShrinkPosition()[1] >= getZoneMatrixDimY() / 2)) {
					try {
						if (isNextPosMoreFar(1, getZoneMatrixDimY(), x, y,
								possibleMoves.get(0))
								&& possibleMoves.get(0) == whichMoveMakesMeLeastFar(
										xShrink, yShrink, x, y))
							result = possibleMoves.get(0);
						else if (isNextPosMoreFar(1, getZoneMatrixDimY(), x, y,
								possibleMoves.get(1))
								&& possibleMoves.get(1) == whichMoveMakesMeLeastFar(
										xShrink, yShrink, x, y))
							result = possibleMoves.get(1);
			...
			// si shrink n'a pas commence encore et si je suis sur la meme case
			// avec un atre personnage
			else if (posPlayer[0] == x && posPlayer[1] == y
					&& lastMove != AI_ACTION_PUT_BOMB
					&& amIcloseToBomb(x, y) > 8) {
			...
			// si je suis proche d'une bombe
			else if (amIcloseToBomb(x, y) < 8
					|| !amIonASafeCase(posBomb[0], posBomb[1], x, y)) {
				...
				if (matrix[x][y] == 4) {
					...
					try {/*
						 ...
						 * if(doYouHaveEmptySideNeighbor(x, y,
						 * possibleMoves.get(i))){ result=possibleMoves.get(i);
						 * break; } else if(doYouHaveEmptyNeighbor(x, y,
						 * possibleMoves.get(i))){ result=possibleMoves.get(i);
						 * break; } else result=lastMove; }
						 */
						for (int i = 0; i < possibleMoves.size(); i++) {
							if (possibleMoves.get(i) == AI_ACTION_GO_UP) {
								...
								} else if (doYouHaveEmptySideNeighbor(x, y,
										possibleMoves.get(i)))
									result = possibleMoves.get(i);
								else if (doYouHaveEmptyNeighbor(x, y,
										possibleMoves.get(i))) {
									result = possibleMoves.get(i);
	...
	private double vraiDistance(int x1, int y1, int x2, int y2) {
		double resulttemp = 0;
		double result;
		resulttemp = Math.pow(Math.abs(x1 - x2), 2)
	...
	private boolean doYouHaveEmptyNeighbor(int x, int y, int move) {
		...
		if (move == AI_ACTION_GO_UP) {
			xNext = x;
			yNext = y - 1;
			if (matrix[xNext][yNext + 1] == 0 || matrix[xNext][yNext - 1] == 0
					|| matrix[xNext - 1][yNext] == 0
					|| matrix[xNext + 1][yNext] == 0)
				result = true;
		}
	...
	private int getPlayerIndex(int xPlayer, int yPlayer) {
		int result = -1;
		for (int i = 0; i < getPlayerCount(); i++) {
			if (getPlayerPosition(i)[0] == xPlayer
					&& getPlayerPosition(i)[1] == yPlayer) {
				return i;
			}
	...
	private int whichMoveMakesMeLeastFar(int x1, int y1, int x2, int y2) {
		...
		for (int i = AI_ACTION_GO_UP; i <= AI_ACTION_GO_RIGHT; i++) {
			if (i == AI_ACTION_GO_UP && isMovePossible(x2, y2, AI_ACTION_GO_UP)
					&& isNextPosMoreFar(x1, y1, x2, y2, AI_ACTION_GO_UP)) {
				if (vraiDistance(x1, y1, x2, y2 - 1) < vraiDist) {
					vraiDist = vraiDistance(x1, y1, x2, y2 - 1);
					flag = i;
				}
			}
			if (i == AI_ACTION_GO_DOWN
					&& isMovePossible(x2, y2, AI_ACTION_GO_DOWN)
					&& isNextPosMoreFar(x1, y1, x2, y2, AI_ACTION_GO_DOWN)) {
				if (vraiDistance(x1, y1, x2, y2 + 1) < vraiDist) {
					vraiDist = vraiDistance(x1, y1, x2, y2 + 1);
					flag = i;
				}
			}
			if (i == AI_ACTION_GO_LEFT
					&& isMovePossible(x2, y2, AI_ACTION_GO_LEFT)
					&& isNextPosMoreFar(x1, y1, x2, y2, AI_ACTION_GO_LEFT)) {
				if (vraiDistance(x1, y1, x2 - 1, y2) < vraiDist) {
					vraiDist = vraiDistance(x1, y1, x2 - 1, y2);
					flag = i;
				}
			}
			if (i == AI_ACTION_GO_RIGHT
					&& isMovePossible(x2, y2, AI_ACTION_GO_RIGHT)
					&& isNextPosMoreFar(x1, y1, x2, y2, AI_ACTION_GO_RIGHT)) {
				if (vraiDistance(x1, y1, x2 + 1, y2) < vraiDist) {
					vraiDist = vraiDistance(x1, y1, x2 + 1, y2);
					flag = i;
				}
	...
	private boolean amIonASafeCase(int x1, int y1, int x2, int y2) {
		boolean result = true;
		if ((x1 == x2 || y1 == y2)
				&& getBombPowerAt(x1, y1) + 2 >= distance(x1, y1, x2, y2)) {
		...
		if (xPlayer == x1) {
			if (yPlayer < y1) {
				...
				for (int i = 1; i < count; i++) {
					if (doYouHaveWallNeighbor(xPlayer, yPlayer + i,
							AI_ACTION_GO_DOWN)) {
						result = true;
						break;
					}
			...
			if (xPlayer < x1) {
				...
				for (int i = 1; i < count; i++) {
					if (doYouHaveWallNeighbor(xPlayer + i, yPlayer,
							AI_ACTION_GO_RIGHT)) {
						result = true;
						break;
					}
	...
	private boolean doYouHaveWallNeighbor(int x, int y, int move) {
		...
		if (move == AI_ACTION_GO_UP) {
			xNext = x;
			yNext = y - 1;
			if (matrix[xNext][yNext] == AI_BLOCK_WALL_HARD
					|| matrix[xNext][yNext] == AI_BLOCK_WALL_SOFT)
				result = true;
		}
		if (move == AI_ACTION_GO_DOWN) {
			xNext = x;
	...
	private boolean doYouHaveEmptySideNeighbor(int x, int y, int move) {
		...
		if (move == AI_ACTION_GO_UP) {
			xNext = x;
			yNext = y - 1;
			if ((matrix[xNext + 1][yNext] == 0 || matrix[xNext - 1][yNext] == 0)
