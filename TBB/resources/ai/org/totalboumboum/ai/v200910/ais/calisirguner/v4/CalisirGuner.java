...
public class CalisirGuner extends ArtificialIntelligence {
	...
	public AiAction processAction() throws StopRequestException {
		...
		if (bomberman != null) {
			...
			if (!bonus && test_sur(caseactuelle) && !attaque0) {
				adversaires();
	...
	/*
	 * private void attaque() throws StopRequestException { // avant tout : test
	 * d'interruption checkInterruption(); AiTile at=null;
	 * 
	 * 
	 * for (int i=0;i<map.width;i++){ checkInterruption(); for (int
	 * j=0;j<map.height;j++){ checkInterruption(); map.setbombeposs(i, j,
	 * bomberman.getBombRange()); List<AiHero> hero = new ArrayList<AiHero>();
	 * hero=zone.getRemainingHeroes(); Iterator<AiHero>it=hero.iterator();
	 * AiHero temp; while (it.hasNext() && !attaque0){ //System.out.println(at);
	 * temp=it.next(); if ( temp!=null &&(!test_chemin(zone.getTile(j, i))||
	 * longueur(bomberman.getCol(),bomberman.getLine(),i,j)<3 ) &&
	 * temp!=bomberman && cases_sures(temp.getTile()).isEmpty() &&
	 * !cases_sures(zone.getTile(j,i)).isEmpty()) { at=zone.getTile(j, i);
	 * attaque0=true; System.out.println(at);
	 * 
	 * }
	 * 
	 * }map.remplir(zone);}} if (attaque0){ if ( at!=null && at!=caseactuelle)
	 * {resultat=at; //System.out.println(at); chemin(); } else if
	 * (at==caseactuelle){ map.setbombeposs(zone.getOwnHero().getCol(),
	 * zone.getOwnHero() .getLine(), bomberman.getBombRange());
	 * 
		...
		if (!destination.isEmpty()) {
			...
			while (iterator.hasNext()) {
				...
				if (meilleur_astar.findPath()) {
					...
					if (opt == res
							&& longueur(bomberman.getCol(),
									bomberman.getLine(), meilleur_resultat
											.getCol(), meilleur_resultat
											.getLine()) == longueur(bomberman
									.getCol(), bomberman.getLine(), option
									.getCol(), option.getLine())) {
