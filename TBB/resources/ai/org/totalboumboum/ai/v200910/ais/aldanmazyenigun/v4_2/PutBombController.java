 ...
 *
 */
public class PutBombController {

		/**
		 * crée un EscapeManager chargé d'amener le personnage au centre d'une case sûre
		 */
		public PutBombController(AldanmazYenigun ai) throws StopRequestException
		{	ai.checkInterruption(); //APPEL OBLIGATOIRE
			...
			AiHero rival = ai.getTargetHero();
			possibleDest = ai.findWallTiles(ownHero.getTile());
			possibleDestRival = ai.findWallTiles(rival.getTile());
			updatePath();
		}
		...
		private List<AiTile> possibleDestRival;
		...
		{	ai.checkInterruption(); //APPEL OBLIGATOIRE
			...
			path = astar.processShortestPath(ai.getOwnHero().getTile(),possibleDestRival);
		...
		{	ai.checkInterruption(); //APPEL OBLIGATOIRE
			...
			else
			{
				...
				//possibleDest.remove(path.getLastTile());
				updatePath();
				AiTile tile = null;
				if(path.getLength()>1)
					tile = path.getTile(1);
		...
		public List<AiTile> getPossibleDest(){
			return possibleDestRival;
		}
	}
