...
{	private static final long serialVersionUID = 1L;
	...
	{	// init
		...
		{	UntitledSubPanelTable listPanel = new UntitledSubPanelTable(width,height,cols,lines,false);
			...
			// data
			int line = 1;
			int roundIndex = panelIndex*(lines-2);
			while(line<lines && roundIndex<names.size())
			{	Color bg = GuiTools.COLOR_TABLE_REGULAR_BACKGROUND;
				String name = names.get(roundIndex);
	...
	{	int result = names.size()/(lines-2);
		if(names.size()%(lines-2)>0)
			result++;
		else if(result==0)
			result = 1;
		return result;
	...
	{	String result = null;
		if(selectedRow!=-1)
		{	int selectedIndex = (selectedRow-1)+currentPage*(lines-2);
